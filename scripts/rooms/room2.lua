...
local thermostat = _storage.get_table("TERMOSTATO_M2")
local SetPointOn = _storage.get_number("SETPOINTON_M2")
local SensorMovbatt = _storage.get_table("SENSOR_MOV_BATT_M2")
local SensorMovElec = _storage.get_table("SENSOR_MOV_ELEC_M2")
local devicesOn = _storage.get_table("ACTUADORES_ON_M2") or {}
local devicesOff = _storage.get_table("ACTUADORES_OFF_M2") or {}
local masterSwitch = _storage.get_table("MASTERSWITCH_ON_M2") or {}
local ModoDispLuces = _storage.get_string("MODODISPLUCES_M2") or false
local ModoThermoInit = _storage.get_string("MOTIONACTIVATOR_M2") or false
local ModoMasterSwitch = _storage.get_string("MODOMASTERSWITCH_M2") or false

-- No usados en plugins de Aula no lleva puerta ni SetPointOFF
-- local ModoSetpoint = _storage.get_string("MODOSETPOINT_M2") or false
-- local SetPointOff = _storage.get_number("SETPOINTOFF_M2")
-- local sensor_puerta = _storage.get_string("SENSOR_PUERTA_M2")
-- local ModoOffPuertaAbierta = _storage.get_string("OFFPUERTAABIERTA_M2") or false

local ModoPluginStatus = _storage.get_string("ModoId2")
local AccionPluginStatus = _storage.get_string("AccionTextoId2")
local EstadoPluginStatus = _storage.get_string("EstadoTextoId2")
local modeButton = (_core.get_item(ModoPluginStatus))

local data = {}
    data.modo = _storage.get_string("Modo2")
    data.type = _storage.get_string("type2")
    data.accion = _storage.get_number("accion2")
    data.TimerID = _storage.get_string("TimerID2")
    data.counting = _storage.get_string("Counting2")
    data.scancycle = _storage.get_string("scanCycle2")
    data.remaining = _storage.get_string("remaining2")
    data.statustext = _storage.get_string("statusText2")
    data.acciontext = _storage.get_string("accionTexto2")
    data.timeraccion = _storage.get_string("timerAccion2")
    data.TimerIdTick = _storage.get_string("TimerIdTick2")
    data.dueTimestamp = _storage.get_number("dueTimestamp2")
    data.timerduration = _storage.get_number("TimerDuration2")
    data.Remaining_ant = _storage.get_number("Remaining_ant2")
    data.previousTimer = _storage.get_number("previousTimer2")
    data.settingAccionId = _storage.get_string("settinAccion2")
...
local TiempoScaner = math.floor((_storage.get_number("TIEMPOSCAN_M2"))/2) -- tiempo a la mitad ejecurar los tiempo del programa
...
Registro("----------------------------------")
Registro("Manejo de Tiempos Reales/2 Aul2: ",verde)
...
function Occupied()
    ...
    Validar("timerAccion2", "Ocupado")
    Validar("scanCycle2", "Ocupado")
    Rutina_Encendido()
end
...
function RoutineOnLuces(item_id)
    local id = {}
    local mode = _storage.get_string("Modo2")
...
function ActualizarAccion(s)
    ...
    local AccionTextoId = _storage.get_string("AccionTextoId2")
    local EstadoTextoId = _storage.get_string("EstadoTextoId2")
    local curValue = _storage.get_number("accion2")
    ...
    if tonumber(s) ~= tonumber(curValue) then
        ...
        if accionText then
            Validar("accionTexto2", accionText)
    ...
    if tonumber(s) ~= -2 then
        ...
        Validar("statusText2", statusText)
...
function EventMovimiento(Sensor)
    ...
    local motion = _storage.get_string("motion2")
    if Sensor ~= nil then
        ...
        for i in ipairs(Sensor) do -- Rutina FOR : conocer estado de SM
            ...
            if sensor_id.value == true then
                local tiempoRestante2 = Tiempo1 - math.abs(data.previousTimer or 0)
                -- Registro("time Restante: " .. tiempoRestante2)
                if motion ~= nil then -- Conocer estado de SM
                    local securityThreat = _storage.get_string("securityThreat2")
                    setItemValue(motion, true)
                    setItemValue(securityThreat, true)
                end

                if _storage.get_number("accion2") == ACCION_SCAN then
                ...
                if _storage.get_number("accion2") == ACCION_LIBRE then
                    CancelTimer()
                    Rutina_On_Thermo()
                    ActualizarAccion(ACCION_OCUPADO)
                    Validar("timerAccion2", "Ocupado")
                ...
                if _storage.get_number("accion2") == ACCION_CICLO_APAGADO then
                    CancelTimer()
                    ActualizarAccion(ACCION_OCUPADO)
                    Validar("timerAccion2","Ocupado")
                    Validar("scanCycle2", "Ocupado")
                    if _storage.get_string("Mode2") ~= MODO_APAGADO then
                ...
                if ValidateRemainingTime(20) then
                    ...
                    if _storage.get_number("accion2") == ACCION_AUTOOFF then
                        CancelTimer()
                        ActualizarAccion(ACCION_SCAN)
                        Validar("timerAccion2", "Scan")
                        StartTimer(Tiempo1)
                        return
                    end
                end
            ...
            if sensor_id.value == false then
                ...
                if _storage.get_number("accion2") == ACCION_OCUPADO then
                    CancelTimer()
                    ActualizarAccion(ACCION_SCAN)
                    Validar("timerAccion2", "Scan")
                    Validar("scanCycle2", "scan_One")
                    StartTimer(Tiempo1)
                    return
                end
            end
...
function EventoMovDeLibre()
    ...
    Validar("timerAccion2", "CicloApagado")
    Validar("scanCycle2", "CicloApagado")
...
function CicloApagado()
    Registro("#2",rojo)
        if (_storage.get_number("accion2") == ACCION_SCAN) and (_storage.get_string("Modo2") ~= MODO_APAGADO) then
                    RoutineOffLuces(devicesOn)
                    RoutineOffLuces(devicesOff)
        end
        CancelTimer()
...
function StartTimer(timerduration)
    local counting = VariablesDeInicio("Counting2", "0")
    if counting == '1' then -- se dejan las comillas para no reemplazar este valor en las demas paginas del code Python
        return false
    end
    Validar("TimerDuration2", timerduration)
    return StartTimeralways()
end

function StartTimeralways()
    local duration = VariablesDeInicio("TimerDuration2", 30)
    local dueTimestamp1 = os.time() + duration
    Validar("dueTimestamp2", dueTimestamp1)
    local status = RemainingUpgrade()
    Validar("Counting2", '1') -- se dejan las comillas para no reemplazar este valor en las demas paginas del code Python
    if data.TimerID ~= "" and status then
        local timerID = data.TimerID
        _storage.set_string("TimerID2", tostring(timerID))
        _timer.set_timeout_with_id(10000, tostring(timerID),
            "HUB:aul.aula_inteligente/scripts/rooms/room2",
            { arg_name = "timer" })
        return true
    else
        Registro("timer sin id")
        local timerID = _timer.set_timeout(10000, "HUB:aul.aula_inteligente/scripts/rooms/room2",
            { arg_name = "timer" })
        _storage.set_string("TimerID2", tostring(timerID))
        return true
    end
...
function TiempoTranscurrido()
    local Timerload = _storage.get_string("remaining2") or 0
...
function CancelTimer()
    ...
    Remaining_ant = 0
    Validar("Counting2", "0")
    Validar("dueTimestamp2", 0)
    Validar("remaining2", "0")
    Validar("TimerDuration2", 0)
    return true
end
...
function RemainingUpgrade()
    local dueTimestamp1 = VariablesDeInicio("dueTimestamp2", 0)
    ...
    Validar("remaining2", "TR" .. restante)
    return remaining > 0
end

function ValidateRemainingTime(time)
    local timercompare = _storage.get_number("TimerDuration2") or 0
...
---- Main and Tick2 :: Funiciones principales
function Main()
    ...
    Registro("::: Manejos Internos ::: Aul2",verde)
    -- Registro("timerId: " .. _json.encode(data.TimerID),verde)
    TimerHoy()
    Registro("Tiempo Scaner= ".. Tiempo1)
    Registro("Tiempo TiempoScanH= ".. Tiempo2)
    Registro("Modo plugin: " .. _json.encode(data.modo).." |-| plugin status and TimerAccion2: " .. data.acciontext,amarillo)
    Registro("ScanCycle1: " .. _json.encode(data.scancycle),verde)
    Registro("TiempoAnterior_2: " .. _json.encode(data.previousTimer),amarillo)
    Registro("TiempoPosterior_2: " .. _json.encode(TimerRemainingV),amarillo)
    ...
    Validar("previousTimer2", tonumber(TimerRemainingV))
...
function Tick2()
    ...
    local counting = VariablesDeInicio("Counting2", "0")
    local timerAccion = VariablesDeInicio("timerAccion2", "0")
...
if ModoPluginStatus then
    ...
    CancelTimer()
    ActualizarAccion(ACCION_SCAN)
    Validar("Modo2", tostring(modeButton.value)) --Actualiza estado del aul con respecto al cambio de estado en web
    Validar("scanCycle2", "scan_One")
    Validar("timerAccion2", "Scan")
...
if AccionPluginStatus then
    ...
    if EstadoTexto and EstadoTexto.value and AccionButton and AccionButton.value then
        if AccionButton.value == "Libre" and EstadoTexto.value == "Ocupado" then
            ...
            Validar("scanCycle2", "LibreForzado")
            Libre()
        end
    else
