...
local thermostat = _storage.get_table("TERMOSTATO_M12")
local SetPointOn = _storage.get_number("SETPOINTON_M12")
local SensorMovbatt = _storage.get_table("SENSOR_MOV_BATT_M12")
local SensorMovElec = _storage.get_table("SENSOR_MOV_ELEC_M12")
local devicesOn = _storage.get_table("ACTUADORES_ON_M12") or {}
local devicesOff = _storage.get_table("ACTUADORES_OFF_M12") or {}
local masterSwitch = _storage.get_table("MASTERSWITCH_ON_M12") or {}
local ModoDispLuces = _storage.get_string("MODODISPLUCES_M12") or false
local ModoThermoInit = _storage.get_string("MOTIONACTIVATOR_M12") or false
local ModoMasterSwitch = _storage.get_string("MODOMASTERSWITCH_M12") or false

-- No usados en plugins de Aula no lleva puerta ni SetPointOFF
-- local ModoSetpoint = _storage.get_string("MODOSETPOINT_M12") or false
-- local SetPointOff = _storage.get_number("SETPOINTOFF_M12")
-- local sensor_puerta = _storage.get_string("SENSOR_PUERTA_M12")
-- local ModoOffPuertaAbierta = _storage.get_string("OFFPUERTAABIERTA_M12") or false

local ModoPluginStatus = _storage.get_string("ModoId12")
local AccionPluginStatus = _storage.get_string("AccionTextoId12")
local EstadoPluginStatus = _storage.get_string("EstadoTextoId12")
local modeButton = (_core.get_item(ModoPluginStatus))

local data = {}
    data.modo = _storage.get_string("Modo12")
    data.type = _storage.get_string("type12")
    data.accion = _storage.get_number("accion12")
    data.TimerID = _storage.get_string("TimerID12")
    data.counting = _storage.get_string("Counting12")
    data.scancycle = _storage.get_string("scanCycle12")
    data.remaining = _storage.get_string("remaining12")
    data.statustext = _storage.get_string("statusText12")
    data.acciontext = _storage.get_string("accionTexto12")
    data.timeraccion = _storage.get_string("timerAccion12")
    data.TimerIdTick = _storage.get_string("TimerIdTick12")
    data.dueTimestamp = _storage.get_number("dueTimestamp12")
    data.timerduration = _storage.get_number("TimerDuration12")
    data.Remaining_ant = _storage.get_number("Remaining_ant12")
    data.previousTimer = _storage.get_number("previousTimer12")
    data.settingAccionId = _storage.get_string("settinAccion12")
...
local TiempoScaner = math.floor((_storage.get_number("TIEMPOSCAN_M12"))/2) -- tiempo a la mitad ejecurar los tiempo del programa
...
Registro("----------------------------------")
Registro("Manejo de Tiempos Reales/2 Aul12: ",verde)
...
function Occupied()
    ...
    Validar("timerAccion12", "Ocupado")
    Validar("scanCycle12", "Ocupado")
    Rutina_Encendido()
end
...
function RoutineOnLuces(item_id)
    local id = {}
    local mode = _storage.get_string("Modo12")
...
function ActualizarAccion(s)
    ...
    local AccionTextoId = _storage.get_string("AccionTextoId12")
    local EstadoTextoId = _storage.get_string("EstadoTextoId12")
    local curValue = _storage.get_number("accion12")
    ...
    if tonumber(s) ~= tonumber(curValue) then
        ...
        if accionText then
            Validar("accionTexto12", accionText)
    ...
    if tonumber(s) ~= -2 then
        ...
        Validar("statusText12", statusText)
...
function EventMovimiento(Sensor)
    ...
    local motion = _storage.get_string("motion12")
    if Sensor ~= nil then
        ...
        for i in ipairs(Sensor) do -- Rutina FOR : conocer estado de SM
            ...
            if sensor_id.value == true then
                local tiempoRestante12 = Tiempo1 - math.abs(data.previousTimer or 0)
                -- Registro("time Restante: " .. tiempoRestante12)
                if motion ~= nil then -- Conocer estado de SM
                    local securityThreat = _storage.get_string("securityThreat12")
                    setItemValue(motion, true)
                    setItemValue(securityThreat, true)
                end

                if _storage.get_number("accion12") == ACCION_SCAN then
                ...
                if _storage.get_number("accion12") == ACCION_LIBRE then
                    CancelTimer()
                    Rutina_On_Thermo()
                    ActualizarAccion(ACCION_OCUPADO)
                    Validar("timerAccion12", "Ocupado")
                ...
                if _storage.get_number("accion12") == ACCION_CICLO_APAGADO then
                    CancelTimer()
                    ActualizarAccion(ACCION_OCUPADO)
                    Validar("timerAccion12","Ocupado")
                    Validar("scanCycle12", "Ocupado")
                    if _storage.get_string("Mode12") ~= MODO_APAGADO then
                ...
                if ValidateRemainingTime(20) then
                    ...
                    if _storage.get_number("accion12") == ACCION_AUTOOFF then
                        CancelTimer()
                        ActualizarAccion(ACCION_SCAN)
                        Validar("timerAccion12", "Scan")
                        StartTimer(Tiempo1)
                        return
                    end
                end
            ...
            if sensor_id.value == false then
                ...
                if _storage.get_number("accion12") == ACCION_OCUPADO then
                    CancelTimer()
                    ActualizarAccion(ACCION_SCAN)
                    Validar("timerAccion12", "Scan")
                    Validar("scanCycle12", "scan_One")
                    StartTimer(Tiempo1)
                    return
                end
            end
...
function EventoMovDeLibre()
    ...
    Validar("timerAccion12", "CicloApagado")
    Validar("scanCycle12", "CicloApagado")
...
function CicloApagado()
    Registro("#12",rojo)
        if (_storage.get_number("accion12") == ACCION_SCAN) and (_storage.get_string("Modo12") ~= MODO_APAGADO) then
                    RoutineOffLuces(devicesOn)
                    RoutineOffLuces(devicesOff)
        end
        CancelTimer()
...
function StartTimer(timerduration)
    local counting = VariablesDeInicio("Counting12", "0")
    if counting == '1' then -- se dejan las comillas para no reemplazar este valor en las demas paginas del code Python
        return false
    end
    Validar("TimerDuration12", timerduration)
    return StartTimeralways()
end

function StartTimeralways()
    local duration = VariablesDeInicio("TimerDuration12", 30)
    local dueTimestamp1 = os.time() + duration
    Validar("dueTimestamp12", dueTimestamp1)
    local status = RemainingUpgrade()
    Validar("Counting12", '1') -- se dejan las comillas para no reemplazar este valor en las demas paginas del code Python
    if data.TimerID ~= "" and status then
        local timerID = data.TimerID
        _storage.set_string("TimerID12", tostring(timerID))
        _timer.set_timeout_with_id(10000, tostring(timerID),
            "HUB:aul.aula_inteligente/scripts/rooms/room12",
            { arg_name = "timer" })
        return true
    else
        Registro("timer sin id")
        local timerID = _timer.set_timeout(10000, "HUB:aul.aula_inteligente/scripts/rooms/room12",
            { arg_name = "timer" })
        _storage.set_string("TimerID12", tostring(timerID))
        return true
    end
...
function TiempoTranscurrido()
    local Timerload = _storage.get_string("remaining12") or 0
...
function CancelTimer()
    ...
    Remaining_ant = 0
    Validar("Counting12", "0")
    Validar("dueTimestamp12", 0)
    Validar("remaining12", "0")
    Validar("TimerDuration12", 0)
    return true
end
...
function RemainingUpgrade()
    local dueTimestamp1 = VariablesDeInicio("dueTimestamp12", 0)
    ...
    Validar("remaining12", "TR" .. restante)
    return remaining > 0
end

function ValidateRemainingTime(time)
    local timercompare = _storage.get_number("TimerDuration12") or 0
...
---- Main and Tick12 :: Funiciones principales
function Main()
    ...
    Registro("::: Manejos Internos ::: Aul12",verde)
    -- Registro("timerId: " .. _json.encode(data.TimerID),verde)
    TimerHoy()
    Registro("Tiempo Scaner= ".. Tiempo1)
    Registro("Tiempo TiempoScanH= ".. Tiempo2)
    Registro("Modo plugin: " .. _json.encode(data.modo).." |-| plugin status and TimerAccion12: " .. data.acciontext,amarillo)
    Registro("ScanCycle1: " .. _json.encode(data.scancycle),verde)
    Registro("TiempoAnterior_12: " .. _json.encode(data.previousTimer),amarillo)
    Registro("TiempoPosterior_12: " .. _json.encode(TimerRemainingV),amarillo)
    ...
    Validar("previousTimer12", tonumber(TimerRemainingV))
...
function Tick12()
    ...
    local counting = VariablesDeInicio("Counting12", "0")
    local timerAccion = VariablesDeInicio("timerAccion12", "0")
...
if ModoPluginStatus then
    ...
    CancelTimer()
    ActualizarAccion(ACCION_SCAN)
    Validar("Modo12", tostring(modeButton.value)) --Actualiza estado del aul con respecto al cambio de estado en web
    Validar("scanCycle12", "scan_One")
    Validar("timerAccion12", "Scan")
...
if AccionPluginStatus then
    ...
    if EstadoTexto and EstadoTexto.value and AccionButton and AccionButton.value then
        if AccionButton.value == "Libre" and EstadoTexto.value == "Ocupado" then
            ...
            Validar("scanCycle12", "LibreForzado")
            Libre()
        end
    else
