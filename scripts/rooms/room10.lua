...
local thermostat = _storage.get_table("TERMOSTATO_M10")
local SetPointOn = _storage.get_number("SETPOINTON_M10")
local SensorMovbatt = _storage.get_table("SENSOR_MOV_BATT_M10")
local SensorMovElec = _storage.get_table("SENSOR_MOV_ELEC_M10")
local devicesOn = _storage.get_table("ACTUADORES_ON_M10") or {}
local devicesOff = _storage.get_table("ACTUADORES_OFF_M10") or {}
local masterSwitch = _storage.get_table("MASTERSWITCH_ON_M10") or {}
local ModoDispLuces = _storage.get_string("MODODISPLUCES_M10") or false
local ModoThermoInit = _storage.get_string("MOTIONACTIVATOR_M10") or false
local ModoMasterSwitch = _storage.get_string("MODOMASTERSWITCH_M10") or false

-- No usados en plugins de Aula no lleva puerta ni SetPointOFF
-- local ModoSetpoint = _storage.get_string("MODOSETPOINT_M10") or false
-- local SetPointOff = _storage.get_number("SETPOINTOFF_M10")
-- local sensor_puerta = _storage.get_string("SENSOR_PUERTA_M10")
-- local ModoOffPuertaAbierta = _storage.get_string("OFFPUERTAABIERTA_M10") or false

local ModoPluginStatus = _storage.get_string("ModoId10")
local AccionPluginStatus = _storage.get_string("AccionTextoId10")
local EstadoPluginStatus = _storage.get_string("EstadoTextoId10")
local modeButton = (_core.get_item(ModoPluginStatus))

local data = {}
    data.modo = _storage.get_string("Modo10")
    data.type = _storage.get_string("type10")
    data.accion = _storage.get_number("accion10")
    data.TimerID = _storage.get_string("TimerID10")
    data.counting = _storage.get_string("Counting10")
    data.scancycle = _storage.get_string("scanCycle10")
    data.remaining = _storage.get_string("remaining10")
    data.statustext = _storage.get_string("statusText10")
    data.acciontext = _storage.get_string("accionTexto10")
    data.timeraccion = _storage.get_string("timerAccion10")
    data.TimerIdTick = _storage.get_string("TimerIdTick10")
    data.dueTimestamp = _storage.get_number("dueTimestamp10")
    data.timerduration = _storage.get_number("TimerDuration10")
    data.Remaining_ant = _storage.get_number("Remaining_ant10")
    data.previousTimer = _storage.get_number("previousTimer10")
    data.settingAccionId = _storage.get_string("settinAccion10")
...
local TiempoScaner = math.floor((_storage.get_number("TIEMPOSCAN_M10"))/2) -- tiempo a la mitad ejecurar los tiempo del programa
...
Registro("----------------------------------")
Registro("Manejo de Tiempos Reales/2 Aul10: ",verde)
...
function Occupied()
    ...
    Validar("timerAccion10", "Ocupado")
    Validar("scanCycle10", "Ocupado")
    Rutina_Encendido()
end
...
function RoutineOnLuces(item_id)
    local id = {}
    local mode = _storage.get_string("Modo10")
...
function ActualizarAccion(s)
    ...
    local AccionTextoId = _storage.get_string("AccionTextoId10")
    local EstadoTextoId = _storage.get_string("EstadoTextoId10")
    local curValue = _storage.get_number("accion10")
    ...
    if tonumber(s) ~= tonumber(curValue) then
        ...
        if accionText then
            Validar("accionTexto10", accionText)
    ...
    if tonumber(s) ~= -2 then
        ...
        Validar("statusText10", statusText)
...
function EventMovimiento(Sensor)
    ...
    local motion = _storage.get_string("motion10")
    if Sensor ~= nil then
        ...
        for i in ipairs(Sensor) do -- Rutina FOR : conocer estado de SM
            ...
            if sensor_id.value == true then
                local tiempoRestante10 = Tiempo1 - math.abs(data.previousTimer or 0)
                -- Registro("time Restante: " .. tiempoRestante10)
                if motion ~= nil then -- Conocer estado de SM
                    local securityThreat = _storage.get_string("securityThreat10")
                    setItemValue(motion, true)
                    setItemValue(securityThreat, true)
                end

                if _storage.get_number("accion10") == ACCION_SCAN then
                ...
                if _storage.get_number("accion10") == ACCION_LIBRE then
                    CancelTimer()
                    Rutina_On_Thermo()
                    ActualizarAccion(ACCION_OCUPADO)
                    Validar("timerAccion10", "Ocupado")
                ...
                if _storage.get_number("accion10") == ACCION_CICLO_APAGADO then
                    CancelTimer()
                    ActualizarAccion(ACCION_OCUPADO)
                    Validar("timerAccion10","Ocupado")
                    Validar("scanCycle10", "Ocupado")
                    if _storage.get_string("Mode10") ~= MODO_APAGADO then
                ...
                if ValidateRemainingTime(20) then
                    ...
                    if _storage.get_number("accion10") == ACCION_AUTOOFF then
                        CancelTimer()
                        ActualizarAccion(ACCION_SCAN)
                        Validar("timerAccion10", "Scan")
                        StartTimer(Tiempo1)
                        return
                    end
                end
            ...
            if sensor_id.value == false then
                ...
                if _storage.get_number("accion10") == ACCION_OCUPADO then
                    CancelTimer()
                    ActualizarAccion(ACCION_SCAN)
                    Validar("timerAccion10", "Scan")
                    Validar("scanCycle10", "scan_One")
                    StartTimer(Tiempo1)
                    return
                end
            end
...
function EventoMovDeLibre()
    ...
    Validar("timerAccion10", "CicloApagado")
    Validar("scanCycle10", "CicloApagado")
...
function CicloApagado()
    Registro("#10",rojo)
        if (_storage.get_number("accion10") == ACCION_SCAN) and (_storage.get_string("Modo10") ~= MODO_APAGADO) then
                    RoutineOffLuces(devicesOn)
                    RoutineOffLuces(devicesOff)
        end
        CancelTimer()
...
function StartTimer(timerduration)
    local counting = VariablesDeInicio("Counting10", "0")
    if counting == '1' then -- se dejan las comillas para no reemplazar este valor en las demas paginas del code Python
        return false
    end
    Validar("TimerDuration10", timerduration)
    return StartTimeralways()
end

function StartTimeralways()
    local duration = VariablesDeInicio("TimerDuration10", 30)
    local dueTimestamp1 = os.time() + duration
    Validar("dueTimestamp10", dueTimestamp1)
    local status = RemainingUpgrade()
    Validar("Counting10", '1') -- se dejan las comillas para no reemplazar este valor en las demas paginas del code Python
    if data.TimerID ~= "" and status then
        local timerID = data.TimerID
        _storage.set_string("TimerID10", tostring(timerID))
        _timer.set_timeout_with_id(10000, tostring(timerID),
            "HUB:aul.aula_inteligente/scripts/rooms/room10",
            { arg_name = "timer" })
        return true
    else
        Registro("timer sin id")
        local timerID = _timer.set_timeout(10000, "HUB:aul.aula_inteligente/scripts/rooms/room10",
            { arg_name = "timer" })
        _storage.set_string("TimerID10", tostring(timerID))
        return true
    end
...
function TiempoTranscurrido()
    local Timerload = _storage.get_string("remaining10") or 0
...
function CancelTimer()
    ...
    Remaining_ant = 0
    Validar("Counting10", "0")
    Validar("dueTimestamp10", 0)
    Validar("remaining10", "0")
    Validar("TimerDuration10", 0)
    return true
end
...
function RemainingUpgrade()
    local dueTimestamp1 = VariablesDeInicio("dueTimestamp10", 0)
    ...
    Validar("remaining10", "TR" .. restante)
    return remaining > 0
end

function ValidateRemainingTime(time)
    local timercompare = _storage.get_number("TimerDuration10") or 0
...
---- Main and Tick10 :: Funiciones principales
function Main()
    ...
    Registro("::: Manejos Internos ::: Aul10",verde)
    -- Registro("timerId: " .. _json.encode(data.TimerID),verde)
    TimerHoy()
    Registro("Tiempo Scaner= ".. Tiempo1)
    Registro("Tiempo TiempoScanH= ".. Tiempo2)
    Registro("Modo plugin: " .. _json.encode(data.modo).." |-| plugin status and TimerAccion10: " .. data.acciontext,amarillo)
    Registro("ScanCycle1: " .. _json.encode(data.scancycle),verde)
    Registro("TiempoAnterior_10: " .. _json.encode(data.previousTimer),amarillo)
    Registro("TiempoPosterior_10: " .. _json.encode(TimerRemainingV),amarillo)
    ...
    Validar("previousTimer10", tonumber(TimerRemainingV))
...
function Tick10()
    ...
    local counting = VariablesDeInicio("Counting10", "0")
    local timerAccion = VariablesDeInicio("timerAccion10", "0")
...
if ModoPluginStatus then
    ...
    CancelTimer()
    ActualizarAccion(ACCION_SCAN)
    Validar("Modo10", tostring(modeButton.value)) --Actualiza estado del aul con respecto al cambio de estado en web
    Validar("scanCycle10", "scan_One")
    Validar("timerAccion10", "Scan")
...
if AccionPluginStatus then
    ...
    if EstadoTexto and EstadoTexto.value and AccionButton and AccionButton.value then
        if AccionButton.value == "Libre" and EstadoTexto.value == "Ocupado" then
            ...
            Validar("scanCycle10", "LibreForzado")
            Libre()
        end
    else
