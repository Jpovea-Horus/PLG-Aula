...
local thermostat = _storage.get_table("TERMOSTATO_M6")
local SetPointOn = _storage.get_number("SETPOINTON_M6")
local SensorMovbatt = _storage.get_table("SENSOR_MOV_BATT_M6")
local SensorMovElec = _storage.get_table("SENSOR_MOV_ELEC_M6")
local devicesOn = _storage.get_table("ACTUADORES_ON_M6") or {}
local devicesOff = _storage.get_table("ACTUADORES_OFF_M6") or {}
local masterSwitch = _storage.get_table("MASTERSWITCH_ON_M6") or {}
local ModoDispLuces = _storage.get_string("MODODISPLUCES_M6") or false
local ModoThermoInit = _storage.get_string("MOTIONACTIVATOR_M6") or false
local ModoMasterSwitch = _storage.get_string("MODOMASTERSWITCH_M6") or false

-- No usados en plugins de Aula no lleva puerta ni SetPointOFF
-- local ModoSetpoint = _storage.get_string("MODOSETPOINT_M6") or false
-- local SetPointOff = _storage.get_number("SETPOINTOFF_M6")
-- local sensor_puerta = _storage.get_string("SENSOR_PUERTA_M6")
-- local ModoOffPuertaAbierta = _storage.get_string("OFFPUERTAABIERTA_M6") or false

local ModoPluginStatus = _storage.get_string("ModoId6")
local AccionPluginStatus = _storage.get_string("AccionTextoId6")
local EstadoPluginStatus = _storage.get_string("EstadoTextoId6")
local modeButton = (_core.get_item(ModoPluginStatus))

local data = {}
    data.modo = _storage.get_string("Modo6")
    data.type = _storage.get_string("type6")
    data.accion = _storage.get_number("accion6")
    data.TimerID = _storage.get_string("TimerID6")
    data.counting = _storage.get_string("Counting6")
    data.scancycle = _storage.get_string("scanCycle6")
    data.remaining = _storage.get_string("remaining6")
    data.statustext = _storage.get_string("statusText6")
    data.acciontext = _storage.get_string("accionTexto6")
    data.timeraccion = _storage.get_string("timerAccion6")
    data.TimerIdTick = _storage.get_string("TimerIdTick6")
    data.dueTimestamp = _storage.get_number("dueTimestamp6")
    data.timerduration = _storage.get_number("TimerDuration6")
    data.Remaining_ant = _storage.get_number("Remaining_ant6")
    data.previousTimer = _storage.get_number("previousTimer6")
    data.settingAccionId = _storage.get_string("settinAccion6")
...
local TiempoScaner = math.floor((_storage.get_number("TIEMPOSCAN_M6"))/2) -- tiempo a la mitad ejecurar los tiempo del programa
...
Registro("----------------------------------")
Registro("Manejo de Tiempos Reales/2 Aul6: ",verde)
...
function Occupied()
    ...
    Validar("timerAccion6", "Ocupado")
    Validar("scanCycle6", "Ocupado")
    Rutina_Encendido()
end
...
function RoutineOnLuces(item_id)
    local id = {}
    local mode = _storage.get_string("Modo6")
...
function ActualizarAccion(s)
    ...
    local AccionTextoId = _storage.get_string("AccionTextoId6")
    local EstadoTextoId = _storage.get_string("EstadoTextoId6")
    local curValue = _storage.get_number("accion6")
    ...
    if tonumber(s) ~= tonumber(curValue) then
        ...
        if accionText then
            Validar("accionTexto6", accionText)
    ...
    if tonumber(s) ~= -2 then
        ...
        Validar("statusText6", statusText)
...
function EventMovimiento(Sensor)
    ...
    local motion = _storage.get_string("motion6")
    if Sensor ~= nil then
        ...
        for i in ipairs(Sensor) do -- Rutina FOR : conocer estado de SM
            ...
            if sensor_id.value == true then
                local tiempoRestante6 = Tiempo1 - math.abs(data.previousTimer or 0)
                -- Registro("time Restante: " .. tiempoRestante6)
                if motion ~= nil then -- Conocer estado de SM
                    local securityThreat = _storage.get_string("securityThreat6")
                    setItemValue(motion, true)
                    setItemValue(securityThreat, true)
                end

                if _storage.get_number("accion6") == ACCION_SCAN then
                ...
                if _storage.get_number("accion6") == ACCION_LIBRE then
                    CancelTimer()
                    Rutina_On_Thermo()
                    ActualizarAccion(ACCION_OCUPADO)
                    Validar("timerAccion6", "Ocupado")
                ...
                if _storage.get_number("accion6") == ACCION_CICLO_APAGADO then
                    CancelTimer()
                    ActualizarAccion(ACCION_OCUPADO)
                    Validar("timerAccion6","Ocupado")
                    Validar("scanCycle6", "Ocupado")
                    if _storage.get_string("Mode6") ~= MODO_APAGADO then
                ...
                if ValidateRemainingTime(20) then
                    ...
                    if _storage.get_number("accion6") == ACCION_AUTOOFF then
                        CancelTimer()
                        ActualizarAccion(ACCION_SCAN)
                        Validar("timerAccion6", "Scan")
                        StartTimer(Tiempo1)
                        return
                    end
                end
            ...
            if sensor_id.value == false then
                ...
                if _storage.get_number("accion6") == ACCION_OCUPADO then
                    CancelTimer()
                    ActualizarAccion(ACCION_SCAN)
                    Validar("timerAccion6", "Scan")
                    Validar("scanCycle6", "scan_One")
                    StartTimer(Tiempo1)
                    return
                end
            end
...
function EventoMovDeLibre()
    ...
    Validar("timerAccion6", "CicloApagado")
    Validar("scanCycle6", "CicloApagado")
...
function CicloApagado()
    Registro("#6",rojo)
        if (_storage.get_number("accion6") == ACCION_SCAN) and (_storage.get_string("Modo6") ~= MODO_APAGADO) then
                    RoutineOffLuces(devicesOn)
                    RoutineOffLuces(devicesOff)
        end
        CancelTimer()
...
function StartTimer(timerduration)
    local counting = VariablesDeInicio("Counting6", "0")
    if counting == '1' then -- se dejan las comillas para no reemplazar este valor en las demas paginas del code Python
        return false
    end
    Validar("TimerDuration6", timerduration)
    return StartTimeralways()
end

function StartTimeralways()
    local duration = VariablesDeInicio("TimerDuration6", 30)
    local dueTimestamp1 = os.time() + duration
    Validar("dueTimestamp6", dueTimestamp1)
    local status = RemainingUpgrade()
    Validar("Counting6", '1') -- se dejan las comillas para no reemplazar este valor en las demas paginas del code Python
    if data.TimerID ~= "" and status then
        local timerID = data.TimerID
        _storage.set_string("TimerID6", tostring(timerID))
        _timer.set_timeout_with_id(10000, tostring(timerID),
            "HUB:aul.aula_inteligente/scripts/rooms/room6",
            { arg_name = "timer" })
        return true
    else
        Registro("timer sin id")
        local timerID = _timer.set_timeout(10000, "HUB:aul.aula_inteligente/scripts/rooms/room6",
            { arg_name = "timer" })
        _storage.set_string("TimerID6", tostring(timerID))
        return true
    end
...
function TiempoTranscurrido()
    local Timerload = _storage.get_string("remaining6") or 0
...
function CancelTimer()
    ...
    Remaining_ant = 0
    Validar("Counting6", "0")
    Validar("dueTimestamp6", 0)
    Validar("remaining6", "0")
    Validar("TimerDuration6", 0)
    return true
end
...
function RemainingUpgrade()
    local dueTimestamp1 = VariablesDeInicio("dueTimestamp6", 0)
    ...
    Validar("remaining6", "TR" .. restante)
    return remaining > 0
end

function ValidateRemainingTime(time)
    local timercompare = _storage.get_number("TimerDuration6") or 0
...
---- Main and Tick6 :: Funiciones principales
function Main()
    ...
    Registro("::: Manejos Internos ::: Aul6",verde)
    -- Registro("timerId: " .. _json.encode(data.TimerID),verde)
    TimerHoy()
    Registro("Tiempo Scaner= ".. Tiempo1)
    Registro("Tiempo TiempoScanH= ".. Tiempo2)
    Registro("Modo plugin: " .. _json.encode(data.modo).." |-| plugin status and TimerAccion6: " .. data.acciontext,amarillo)
    Registro("ScanCycle1: " .. _json.encode(data.scancycle),verde)
    Registro("TiempoAnterior_6: " .. _json.encode(data.previousTimer),amarillo)
    Registro("TiempoPosterior_6: " .. _json.encode(TimerRemainingV),amarillo)
    ...
    Validar("previousTimer6", tonumber(TimerRemainingV))
...
function Tick6()
    ...
    local counting = VariablesDeInicio("Counting6", "0")
    local timerAccion = VariablesDeInicio("timerAccion6", "0")
...
if ModoPluginStatus then
    ...
    CancelTimer()
    ActualizarAccion(ACCION_SCAN)
    Validar("Modo6", tostring(modeButton.value)) --Actualiza estado del aul con respecto al cambio de estado en web
    Validar("scanCycle6", "scan_One")
    Validar("timerAccion6", "Scan")
...
if AccionPluginStatus then
    ...
    if EstadoTexto and EstadoTexto.value and AccionButton and AccionButton.value then
        if AccionButton.value == "Libre" and EstadoTexto.value == "Ocupado" then
            ...
            Validar("scanCycle6", "LibreForzado")
            Libre()
        end
    else
