...
local thermostat = _storage.get_table("TERMOSTATO_M1")
local SetPointOn = _storage.get_number("SETPOINTON_M1")
local SensorMovbatt = _storage.get_table("SENSOR_MOV_BATT_M1")
local SensorMovElec = _storage.get_table("SENSOR_MOV_ELEC_M1")
local devicesOn = _storage.get_table("ACTUADORES_ON_M1") or {}
local devicesOff = _storage.get_table("ACTUADORES_OFF_M1") or {}
local masterSwitch = _storage.get_table("MASTERSWITCH_ON_M1") or {}
local ModoDispLuces = _storage.get_string("MODODISPLUCES_M1") or false
local ModoThermoInit = _storage.get_string("MOTIONACTIVATOR_M1") or false
local ModoMasterSwitch = _storage.get_string("MODOMASTERSWITCH_M1") or false

-- No usados en plugins de Aula no lleva puerta ni SetPointOFF
-- local ModoSetpoint = _storage.get_string("MODOSETPOINT_M1") or false
-- local SetPointOff = _storage.get_number("SETPOINTOFF_M1")
-- local sensor_puerta = _storage.get_string("SENSOR_PUERTA_M1")
-- local ModoOffPuertaAbierta = _storage.get_string("OFFPUERTAABIERTA_M1") or false
...
local data = {}
    data.modo = _storage.get_string("Modo1")
...
local TiempoScaner = math.floor((_storage.get_number("TIEMPOSCAN_M1"))/2) -- tiempo a la mitad ejecurar los tiempo del programa
...
Registro("----------------------------------")
Registro("Manejo de Tiempos Reales/2 Aul1: ",verde)
...
function Occupied()
    ...
    Validar("timerAccion1", "Ocupado")
    Validar("scanCycle1", "Ocupado")
    Rutina_Encendido()
end
...
function RoutineOnLuces(item_id)
    local id = {}
    local mode = _storage.get_string("Modo1")
...
function ActualizarAccion(s)
    ...
    local AccionTextoId = _storage.get_string("AccionTextoId1")
    local EstadoTextoId = _storage.get_string("EstadoTextoId1")
    local curValue = _storage.get_number("accion1")
    ...
    if tonumber(s) ~= tonumber(curValue) then
        ...
        if accionText then
            Validar("accionTexto1", accionText)
    ...
    if tonumber(s) ~= -2 then
        ...
        Validar("statusText1", statusText)
...
function EventMovimiento(Sensor)
    ...
    local motion = _storage.get_string("motion1")
    if Sensor ~= nil then
        ...
        for i in ipairs(Sensor) do -- Rutina FOR : conocer estado de SM
            ...
            if sensor_id.value == true then
                ...
                if motion ~= nil then -- Conocer estado de SM
                    local securityThreat = _storage.get_string("securityThreat1")
                    setItemValue(motion, true)
                    setItemValue(securityThreat, true)
                end

                if _storage.get_number("accion1") == ACCION_SCAN then
                ...
                if _storage.get_number("accion1") == ACCION_LIBRE then
                    CancelTimer()
                    Rutina_On_Thermo()
                    ActualizarAccion(ACCION_OCUPADO)
                    Validar("timerAccion1", "Ocupado")
                ...
                if _storage.get_number("accion1") == ACCION_CICLO_APAGADO then
                    CancelTimer()
                    ActualizarAccion(ACCION_OCUPADO)
                    Validar("timerAccion1","Ocupado")
                    Validar("scanCycle1", "Ocupado")
                    if _storage.get_string("Mode1") ~= MODO_APAGADO then
                ...
                if ValidateRemainingTime(20) then
                    ...
                    if _storage.get_number("accion1") == ACCION_AUTOOFF then
                        CancelTimer()
                        ActualizarAccion(ACCION_SCAN)
                        Validar("timerAccion1", "Scan")
                        StartTimer(Tiempo1)
                        return
                    end
                end
            ...
            if sensor_id.value == false then
                ...
                if _storage.get_number("accion1") == ACCION_OCUPADO then
                    CancelTimer()
                    ActualizarAccion(ACCION_SCAN)
                    Validar("timerAccion1", "Scan")
                    Validar("scanCycle1", "scan_One")
                    StartTimer(Tiempo1)
                    return
                end
            end
...
function EventoMovDeLibre()
    ...
    Validar("timerAccion1", "CicloApagado")
    Validar("scanCycle1", "CicloApagado")
...
function EventoActuadores(Actuador)
    ...
    local tiempoRestante1 = Tiempo1 - math.abs(data.previousTimer or 0)
    -- Registro("time Restante: " .. tiempoRestante1)
...
function CicloApagado()
    Registro("#2",rojo)
        if (_storage.get_number("accion1") == ACCION_SCAN) and (_storage.get_string("Modo1") ~= MODO_APAGADO) then
                    RoutineOffLuces(devicesOn)
                    RoutineOffLuces(devicesOff)
        end
        CancelTimer()
...
function StartTimer(timerduration)
    local counting = VariablesDeInicio("Counting1", "0")
    if counting == '1' then -- se dejan las comillas para no reemplazar este valor en las demas paginas del code Python
        return false
    end
    Validar("TimerDuration1", timerduration)
    return StartTimeralways()
end

function StartTimeralways()
    local duration = VariablesDeInicio("TimerDuration1", 30)
    local dueTimestamp1 = os.time() + duration
    Validar("dueTimestamp1", dueTimestamp1)
    local status = RemainingUpgrade()
    Validar("Counting1", '1') -- se dejan las comillas para no reemplazar este valor en las demas paginas del code Python
    if data.TimerID ~= "" and status then
        local timerID = data.TimerID
        _storage.set_string("TimerID1", tostring(timerID))
        _timer.set_timeout_with_id(10000, tostring(timerID),
            "HUB:aul.aula_inteligente/scripts/rooms/room1",
            { arg_name = "timer" })
        return true
    else
        Registro("timer sin id")
        local timerID = _timer.set_timeout(10000, "HUB:aul.aula_inteligente/scripts/rooms/room1",
            { arg_name = "timer" })
        _storage.set_string("TimerID1", tostring(timerID))
        return true
    end
...
function TiempoTranscurrido()
    local Timerload = _storage.get_string("remaining1") or 0
...
function CancelTimer()
    ...
    Remaining_ant = 0
    Validar("Counting1", "0")
    Validar("dueTimestamp1", 0)
    Validar("remaining1", "0")
    Validar("TimerDuration1", 0)
    return true
end
...
function RemainingUpgrade()
    local dueTimestamp1 = VariablesDeInicio("dueTimestamp1", 0)
    ...
    Validar("remaining1", "TR" .. restante)
    return remaining > 0
end

function ValidateRemainingTime(time)
    local timercompare = _storage.get_number("TimerDuration1") or 0
...
---- Main and Tick1 :: Funiciones principales
function Main()
    ...
    Registro("::: Manejos Internos ::: Aul1",verde)
    -- Registro("timerId: " .. _json.encode(data.TimerID),verde)
    TimerHoy()
    Registro("Tiempo Scaner= ".. Tiempo1)
    Registro("Tiempo TiempoScanH= ".. Tiempo2)
    Registro("Modo plugin: " .. _json.encode(data.modo).." |-| plugin status and TimerAccion1: " .. data.acciontext,amarillo)
    Registro("ScanCycle1: " .. _json.encode(data.scancycle),verde)
    Registro("TiempoAnterior_1: " .. _json.encode(data.previousTimer),amarillo)
    Registro("TiempoPosterior_1: " .. _json.encode(TimerRemainingV),amarillo)
    ...
    Validar("previousTimer1", tonumber(TimerRemainingV))
...
function Tick1()
    ...
    local counting = VariablesDeInicio("Counting1", "0")
    local timerAccion = VariablesDeInicio("timerAccion1", "0")
...
if ModoPluginStatus then
    ...
    CancelTimer()
    ActualizarAccion(ACCION_SCAN)
    Validar("Modo1", tostring(modeButton.value)) --Actualiza estado del aul con respecto al cambio de estado en web
    Validar("scanCycle1", "scan_One")
    Validar("timerAccion1", "Scan")
...
if AccionPluginStatus then
    ...
    if EstadoTexto and EstadoTexto.value and AccionButton and AccionButton.value then
        if AccionButton.value == "Libre" and EstadoTexto.value == "Ocupado" then
            ...
            Validar("scanCycle1", "LibreForzado")
            Libre()
        end
    else
