...
local thermostat = _storage.get_table("TERMOSTATO_M14")
local SetPointOn = _storage.get_number("SETPOINTON_M14")
local SensorMovbatt = _storage.get_table("SENSOR_MOV_BATT_M14")
local SensorMovElec = _storage.get_table("SENSOR_MOV_ELEC_M14")
local devicesOn = _storage.get_table("ACTUADORES_ON_M14") or {}
local devicesOff = _storage.get_table("ACTUADORES_OFF_M14") or {}
local masterSwitch = _storage.get_table("MASTERSWITCH_ON_M14") or {}
local ModoDispLuces = _storage.get_string("MODODISPLUCES_M14") or false
local ModoThermoInit = _storage.get_string("MOTIONACTIVATOR_M14") or false
local ModoMasterSwitch = _storage.get_string("MODOMASTERSWITCH_M14") or false

-- No usados en plugins de Aula no lleva puerta ni SetPointOFF
-- local ModoSetpoint = _storage.get_string("MODOSETPOINT_M14") or false
-- local SetPointOff = _storage.get_number("SETPOINTOFF_M14")
-- local sensor_puerta = _storage.get_string("SENSOR_PUERTA_M14")
-- local ModoOffPuertaAbierta = _storage.get_string("OFFPUERTAABIERTA_M14") or false

local ModoPluginStatus = _storage.get_string("ModoId14")
local AccionPluginStatus = _storage.get_string("AccionTextoId14")
local EstadoPluginStatus = _storage.get_string("EstadoTextoId14")
local modeButton = (_core.get_item(ModoPluginStatus))

local data = {}
    data.modo = _storage.get_string("Modo14")
    data.type = _storage.get_string("type14")
    data.accion = _storage.get_number("accion14")
    data.TimerID = _storage.get_string("TimerID14")
    data.counting = _storage.get_string("Counting14")
    data.scancycle = _storage.get_string("scanCycle14")
    data.remaining = _storage.get_string("remaining14")
    data.statustext = _storage.get_string("statusText14")
    data.acciontext = _storage.get_string("accionTexto14")
    data.timeraccion = _storage.get_string("timerAccion14")
    data.TimerIdTick = _storage.get_string("TimerIdTick14")
    data.dueTimestamp = _storage.get_number("dueTimestamp14")
    data.timerduration = _storage.get_number("TimerDuration14")
    data.Remaining_ant = _storage.get_number("Remaining_ant14")
    data.previousTimer = _storage.get_number("previousTimer14")
    data.settingAccionId = _storage.get_string("settinAccion14")
...
local TiempoScaner = math.floor((_storage.get_number("TIEMPOSCAN_M14"))/2) -- tiempo a la mitad ejecurar los tiempo del programa
...
Registro("----------------------------------")
Registro("Manejo de Tiempos Reales/2 Aul14: ",verde)
...
function Occupied()
    ...
    Validar("timerAccion14", "Ocupado")
    Validar("scanCycle14", "Ocupado")
    Rutina_Encendido()
end
...
function RoutineOnLuces(item_id)
    local id = {}
    local mode = _storage.get_string("Modo14")
...
function ActualizarAccion(s)
    ...
    local AccionTextoId = _storage.get_string("AccionTextoId14")
    local EstadoTextoId = _storage.get_string("EstadoTextoId14")
    local curValue = _storage.get_number("accion14")
    ...
    if tonumber(s) ~= tonumber(curValue) then
        ...
        if accionText then
            Validar("accionTexto14", accionText)
    ...
    if tonumber(s) ~= -2 then
        ...
        Validar("statusText14", statusText)
...
function EventMovimiento(Sensor)
    ...
    local motion = _storage.get_string("motion14")
    if Sensor ~= nil then
        ...
        for i in ipairs(Sensor) do -- Rutina FOR : conocer estado de SM
            ...
            if sensor_id.value == true then
                local tiempoRestante14 = Tiempo1 - math.abs(data.previousTimer or 0)
                -- Registro("time Restante: " .. tiempoRestante14)
                if motion ~= nil then -- Conocer estado de SM
                    local securityThreat = _storage.get_string("securityThreat14")
                    setItemValue(motion, true)
                    setItemValue(securityThreat, true)
                end

                if _storage.get_number("accion14") == ACCION_SCAN then
                ...
                if _storage.get_number("accion14") == ACCION_LIBRE then
                    CancelTimer()
                    Rutina_On_Thermo()
                    ActualizarAccion(ACCION_OCUPADO)
                    Validar("timerAccion14", "Ocupado")
                ...
                if _storage.get_number("accion14") == ACCION_CICLO_APAGADO then
                    CancelTimer()
                    ActualizarAccion(ACCION_OCUPADO)
                    Validar("timerAccion14","Ocupado")
                    Validar("scanCycle14", "Ocupado")
                    if _storage.get_string("Mode14") ~= MODO_APAGADO then
                ...
                if ValidateRemainingTime(20) then
                    ...
                    if _storage.get_number("accion14") == ACCION_AUTOOFF then
                        CancelTimer()
                        ActualizarAccion(ACCION_SCAN)
                        Validar("timerAccion14", "Scan")
                        StartTimer(Tiempo1)
                        return
                    end
                end
            ...
            if sensor_id.value == false then
                ...
                if _storage.get_number("accion14") == ACCION_OCUPADO then
                    CancelTimer()
                    ActualizarAccion(ACCION_SCAN)
                    Validar("timerAccion14", "Scan")
                    Validar("scanCycle14", "scan_One")
                    StartTimer(Tiempo1)
                    return
                end
            end
...
function EventoMovDeLibre()
    ...
    Validar("timerAccion14", "CicloApagado")
    Validar("scanCycle14", "CicloApagado")
...
function CicloApagado()
    Registro("#14",rojo)
        if (_storage.get_number("accion14") == ACCION_SCAN) and (_storage.get_string("Modo14") ~= MODO_APAGADO) then
                    RoutineOffLuces(devicesOn)
                    RoutineOffLuces(devicesOff)
        end
        CancelTimer()
...
function StartTimer(timerduration)
    local counting = VariablesDeInicio("Counting14", "0")
    if counting == '1' then -- se dejan las comillas para no reemplazar este valor en las demas paginas del code Python
        return false
    end
    Validar("TimerDuration14", timerduration)
    return StartTimeralways()
end

function StartTimeralways()
    local duration = VariablesDeInicio("TimerDuration14", 30)
    local dueTimestamp1 = os.time() + duration
    Validar("dueTimestamp14", dueTimestamp1)
    local status = RemainingUpgrade()
    Validar("Counting14", '1') -- se dejan las comillas para no reemplazar este valor en las demas paginas del code Python
    if data.TimerID ~= "" and status then
        local timerID = data.TimerID
        _storage.set_string("TimerID14", tostring(timerID))
        _timer.set_timeout_with_id(10000, tostring(timerID),
            "HUB:aul.aula_inteligente/scripts/rooms/room14",
            { arg_name = "timer" })
        return true
    else
        Registro("timer sin id")
        local timerID = _timer.set_timeout(10000, "HUB:aul.aula_inteligente/scripts/rooms/room14",
            { arg_name = "timer" })
        _storage.set_string("TimerID14", tostring(timerID))
        return true
    end
...
function TiempoTranscurrido()
    local Timerload = _storage.get_string("remaining14") or 0
...
function CancelTimer()
    ...
    Remaining_ant = 0
    Validar("Counting14", "0")
    Validar("dueTimestamp14", 0)
    Validar("remaining14", "0")
    Validar("TimerDuration14", 0)
    return true
end
...
function RemainingUpgrade()
    local dueTimestamp1 = VariablesDeInicio("dueTimestamp14", 0)
    ...
    Validar("remaining14", "TR" .. restante)
    return remaining > 0
end

function ValidateRemainingTime(time)
    local timercompare = _storage.get_number("TimerDuration14") or 0
...
---- Main and Tick14 :: Funiciones principales
function Main()
    ...
    Registro("::: Manejos Internos ::: Aul14",verde)
    -- Registro("timerId: " .. _json.encode(data.TimerID),verde)
    TimerHoy()
    Registro("Tiempo Scaner= ".. Tiempo1)
    Registro("Tiempo TiempoScanH= ".. Tiempo2)
    Registro("Modo plugin: " .. _json.encode(data.modo).." |-| plugin status and TimerAccion14: " .. data.acciontext,amarillo)
    Registro("ScanCycle1: " .. _json.encode(data.scancycle),verde)
    Registro("TiempoAnterior_14: " .. _json.encode(data.previousTimer),amarillo)
    Registro("TiempoPosterior_14: " .. _json.encode(TimerRemainingV),amarillo)
    ...
    Validar("previousTimer14", tonumber(TimerRemainingV))
...
function Tick14()
    ...
    local counting = VariablesDeInicio("Counting14", "0")
    local timerAccion = VariablesDeInicio("timerAccion14", "0")
...
if ModoPluginStatus then
    ...
    CancelTimer()
    ActualizarAccion(ACCION_SCAN)
    Validar("Modo14", tostring(modeButton.value)) --Actualiza estado del aul con respecto al cambio de estado en web
    Validar("scanCycle14", "scan_One")
    Validar("timerAccion14", "Scan")
...
if AccionPluginStatus then
    ...
    if EstadoTexto and EstadoTexto.value and AccionButton and AccionButton.value then
        if AccionButton.value == "Libre" and EstadoTexto.value == "Ocupado" then
            ...
            Validar("scanCycle14", "LibreForzado")
            Libre()
        end
    else
