...
local thermostat = _storage.get_table("TERMOSTATO_M15")
local SetPointOn = _storage.get_number("SETPOINTON_M15")
local SensorMovbatt = _storage.get_table("SENSOR_MOV_BATT_M15")
local SensorMovElec = _storage.get_table("SENSOR_MOV_ELEC_M15")
local devicesOn = _storage.get_table("ACTUADORES_ON_M15") or {}
local devicesOff = _storage.get_table("ACTUADORES_OFF_M15") or {}
local masterSwitch = _storage.get_table("MASTERSWITCH_ON_M15") or {}
local ModoDispLuces = _storage.get_string("MODODISPLUCES_M15") or false
local ModoThermoInit = _storage.get_string("MOTIONACTIVATOR_M15") or false
local ModoMasterSwitch = _storage.get_string("MODOMASTERSWITCH_M15") or false

-- No usados en plugins de Aula no lleva puerta ni SetPointOFF
-- local ModoSetpoint = _storage.get_string("MODOSETPOINT_M15") or false
-- local SetPointOff = _storage.get_number("SETPOINTOFF_M15")
-- local sensor_puerta = _storage.get_string("SENSOR_PUERTA_M15")
-- local ModoOffPuertaAbierta = _storage.get_string("OFFPUERTAABIERTA_M15") or false

local ModoPluginStatus = _storage.get_string("ModoId15")
local AccionPluginStatus = _storage.get_string("AccionTextoId15")
local EstadoPluginStatus = _storage.get_string("EstadoTextoId15")
local modeButton = (_core.get_item(ModoPluginStatus))

local data = {}
    data.modo = _storage.get_string("Modo15")
    data.type = _storage.get_string("type15")
    data.accion = _storage.get_number("accion15")
    data.TimerID = _storage.get_string("TimerID15")
    data.counting = _storage.get_string("Counting15")
    data.scancycle = _storage.get_string("scanCycle15")
    data.remaining = _storage.get_string("remaining15")
    data.statustext = _storage.get_string("statusText15")
    data.acciontext = _storage.get_string("accionTexto15")
    data.timeraccion = _storage.get_string("timerAccion15")
    data.TimerIdTick = _storage.get_string("TimerIdTick15")
    data.dueTimestamp = _storage.get_number("dueTimestamp15")
    data.timerduration = _storage.get_number("TimerDuration15")
    data.Remaining_ant = _storage.get_number("Remaining_ant15")
    data.previousTimer = _storage.get_number("previousTimer15")
    data.settingAccionId = _storage.get_string("settinAccion15")
...
local TiempoScaner = math.floor((_storage.get_number("TIEMPOSCAN_M15"))/2) -- tiempo a la mitad ejecurar los tiempo del programa
...
Registro("----------------------------------")
Registro("Manejo de Tiempos Reales/2 Aul15: ",verde)
...
function Occupied()
    ...
    Validar("timerAccion15", "Ocupado")
    Validar("scanCycle15", "Ocupado")
    Rutina_Encendido()
end
...
function RoutineOnLuces(item_id)
    local id = {}
    local mode = _storage.get_string("Modo15")
...
function ActualizarAccion(s)
    ...
    local AccionTextoId = _storage.get_string("AccionTextoId15")
    local EstadoTextoId = _storage.get_string("EstadoTextoId15")
    local curValue = _storage.get_number("accion15")
    ...
    if tonumber(s) ~= tonumber(curValue) then
        ...
        if accionText then
            Validar("accionTexto15", accionText)
    ...
    if tonumber(s) ~= -2 then
        ...
        Validar("statusText15", statusText)
...
function EventMovimiento(Sensor)
    ...
    local motion = _storage.get_string("motion15")
    if Sensor ~= nil then
        ...
        for i in ipairs(Sensor) do -- Rutina FOR : conocer estado de SM
            ...
            if sensor_id.value == true then
                local tiempoRestante15 = Tiempo1 - math.abs(data.previousTimer or 0)
                -- Registro("time Restante: " .. tiempoRestante15)
                if motion ~= nil then -- Conocer estado de SM
                    local securityThreat = _storage.get_string("securityThreat15")
                    setItemValue(motion, true)
                    setItemValue(securityThreat, true)
                end

                if _storage.get_number("accion15") == ACCION_SCAN then
                ...
                if _storage.get_number("accion15") == ACCION_LIBRE then
                    CancelTimer()
                    Rutina_On_Thermo()
                    ActualizarAccion(ACCION_OCUPADO)
                    Validar("timerAccion15", "Ocupado")
                ...
                if _storage.get_number("accion15") == ACCION_CICLO_APAGADO then
                    CancelTimer()
                    ActualizarAccion(ACCION_OCUPADO)
                    Validar("timerAccion15","Ocupado")
                    Validar("scanCycle15", "Ocupado")
                    if _storage.get_string("Mode15") ~= MODO_APAGADO then
                ...
                if ValidateRemainingTime(20) then
                    ...
                    if _storage.get_number("accion15") == ACCION_AUTOOFF then
                        CancelTimer()
                        ActualizarAccion(ACCION_SCAN)
                        Validar("timerAccion15", "Scan")
                        StartTimer(Tiempo1)
                        return
                    end
                end
            ...
            if sensor_id.value == false then
                ...
                if _storage.get_number("accion15") == ACCION_OCUPADO then
                    CancelTimer()
                    ActualizarAccion(ACCION_SCAN)
                    Validar("timerAccion15", "Scan")
                    Validar("scanCycle15", "scan_One")
                    StartTimer(Tiempo1)
                    return
                end
            end
...
function EventoMovDeLibre()
    ...
    Validar("timerAccion15", "CicloApagado")
    Validar("scanCycle15", "CicloApagado")
...
function CicloApagado()
    Registro("#15",rojo)
        if (_storage.get_number("accion15") == ACCION_SCAN) and (_storage.get_string("Modo15") ~= MODO_APAGADO) then
                    RoutineOffLuces(devicesOn)
                    RoutineOffLuces(devicesOff)
        end
        CancelTimer()
...
function StartTimer(timerduration)
    local counting = VariablesDeInicio("Counting15", "0")
    if counting == '1' then -- se dejan las comillas para no reemplazar este valor en las demas paginas del code Python
        return false
    end
    Validar("TimerDuration15", timerduration)
    return StartTimeralways()
end

function StartTimeralways()
    local duration = VariablesDeInicio("TimerDuration15", 30)
    local dueTimestamp1 = os.time() + duration
    Validar("dueTimestamp15", dueTimestamp1)
    local status = RemainingUpgrade()
    Validar("Counting15", '1') -- se dejan las comillas para no reemplazar este valor en las demas paginas del code Python
    if data.TimerID ~= "" and status then
        local timerID = data.TimerID
        _storage.set_string("TimerID15", tostring(timerID))
        _timer.set_timeout_with_id(10000, tostring(timerID),
            "HUB:aul.aula_inteligente/scripts/rooms/room15",
            { arg_name = "timer" })
        return true
    else
        Registro("timer sin id")
        local timerID = _timer.set_timeout(10000, "HUB:aul.aula_inteligente/scripts/rooms/room15",
            { arg_name = "timer" })
        _storage.set_string("TimerID15", tostring(timerID))
        return true
    end
...
function TiempoTranscurrido()
    local Timerload = _storage.get_string("remaining15") or 0
...
function CancelTimer()
    ...
    Remaining_ant = 0
    Validar("Counting15", "0")
    Validar("dueTimestamp15", 0)
    Validar("remaining15", "0")
    Validar("TimerDuration15", 0)
    return true
end
...
function RemainingUpgrade()
    local dueTimestamp1 = VariablesDeInicio("dueTimestamp15", 0)
    ...
    Validar("remaining15", "TR" .. restante)
    return remaining > 0
end

function ValidateRemainingTime(time)
    local timercompare = _storage.get_number("TimerDuration15") or 0
...
---- Main and Tick15 :: Funiciones principales
function Main()
    ...
    Registro("::: Manejos Internos ::: Aul15",verde)
    -- Registro("timerId: " .. _json.encode(data.TimerID),verde)
    TimerHoy()
    Registro("Tiempo Scaner= ".. Tiempo1)
    Registro("Tiempo TiempoScanH= ".. Tiempo2)
    Registro("Modo plugin: " .. _json.encode(data.modo).." |-| plugin status and TimerAccion15: " .. data.acciontext,amarillo)
    Registro("ScanCycle1: " .. _json.encode(data.scancycle),verde)
    Registro("TiempoAnterior_15: " .. _json.encode(data.previousTimer),amarillo)
    Registro("TiempoPosterior_15: " .. _json.encode(TimerRemainingV),amarillo)
    ...
    Validar("previousTimer15", tonumber(TimerRemainingV))
...
function Tick15()
    ...
    local counting = VariablesDeInicio("Counting15", "0")
    local timerAccion = VariablesDeInicio("timerAccion15", "0")
...
if ModoPluginStatus then
    ...
    CancelTimer()
    ActualizarAccion(ACCION_SCAN)
    Validar("Modo15", tostring(modeButton.value)) --Actualiza estado del aul con respecto al cambio de estado en web
    Validar("scanCycle15", "scan_One")
    Validar("timerAccion15", "Scan")
...
if AccionPluginStatus then
    ...
    if EstadoTexto and EstadoTexto.value and AccionButton and AccionButton.value then
        if AccionButton.value == "Libre" and EstadoTexto.value == "Ocupado" then
            ...
            Validar("scanCycle15", "LibreForzado")
            Libre()
        end
    else
