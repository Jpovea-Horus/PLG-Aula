...
local thermostat = _storage.get_table("TERMOSTATO_M7")
local SetPointOn = _storage.get_number("SETPOINTON_M7")
local SensorMovbatt = _storage.get_table("SENSOR_MOV_BATT_M7")
local SensorMovElec = _storage.get_table("SENSOR_MOV_ELEC_M7")
local devicesOn = _storage.get_table("ACTUADORES_ON_M7") or {}
local devicesOff = _storage.get_table("ACTUADORES_OFF_M7") or {}
local masterSwitch = _storage.get_table("MASTERSWITCH_ON_M7") or {}
local ModoDispLuces = _storage.get_string("MODODISPLUCES_M7") or false
local ModoThermoInit = _storage.get_string("MOTIONACTIVATOR_M7") or false
local ModoMasterSwitch = _storage.get_string("MODOMASTERSWITCH_M7") or false

-- No usados en plugins de Aula no lleva puerta ni SetPointOFF
-- local ModoSetpoint = _storage.get_string("MODOSETPOINT_M7") or false
-- local SetPointOff = _storage.get_number("SETPOINTOFF_M7")
-- local sensor_puerta = _storage.get_string("SENSOR_PUERTA_M7")
-- local ModoOffPuertaAbierta = _storage.get_string("OFFPUERTAABIERTA_M7") or false

local ModoPluginStatus = _storage.get_string("ModoId7")
local AccionPluginStatus = _storage.get_string("AccionTextoId7")
local EstadoPluginStatus = _storage.get_string("EstadoTextoId7")
local modeButton = (_core.get_item(ModoPluginStatus))

local data = {}
    data.modo = _storage.get_string("Modo7")
    data.type = _storage.get_string("type7")
    data.accion = _storage.get_number("accion7")
    data.TimerID = _storage.get_string("TimerID7")
    data.counting = _storage.get_string("Counting7")
    data.scancycle = _storage.get_string("scanCycle7")
    data.remaining = _storage.get_string("remaining7")
    data.statustext = _storage.get_string("statusText7")
    data.acciontext = _storage.get_string("accionTexto7")
    data.timeraccion = _storage.get_string("timerAccion7")
    data.TimerIdTick = _storage.get_string("TimerIdTick7")
    data.dueTimestamp = _storage.get_number("dueTimestamp7")
    data.timerduration = _storage.get_number("TimerDuration7")
    data.Remaining_ant = _storage.get_number("Remaining_ant7")
    data.previousTimer = _storage.get_number("previousTimer7")
    data.settingAccionId = _storage.get_string("settinAccion7")
...
local TiempoScaner = math.floor((_storage.get_number("TIEMPOSCAN_M7"))/2) -- tiempo a la mitad ejecurar los tiempo del programa
...
Registro("----------------------------------")
Registro("Manejo de Tiempos Reales/2 Aul7: ",verde)
...
function Occupied()
    ...
    Validar("timerAccion7", "Ocupado")
    Validar("scanCycle7", "Ocupado")
    Rutina_Encendido()
end
...
function RoutineOnLuces(item_id)
    local id = {}
    local mode = _storage.get_string("Modo7")
...
function ActualizarAccion(s)
    ...
    local AccionTextoId = _storage.get_string("AccionTextoId7")
    local EstadoTextoId = _storage.get_string("EstadoTextoId7")
    local curValue = _storage.get_number("accion7")
    ...
    if tonumber(s) ~= tonumber(curValue) then
        ...
        if accionText then
            Validar("accionTexto7", accionText)
    ...
    if tonumber(s) ~= -2 then
        ...
        Validar("statusText7", statusText)
...
function EventMovimiento(Sensor)
    ...
    local motion = _storage.get_string("motion7")
    if Sensor ~= nil then
        ...
        for i in ipairs(Sensor) do -- Rutina FOR : conocer estado de SM
            ...
            if sensor_id.value == true then
                local tiempoRestante7 = Tiempo1 - math.abs(data.previousTimer or 0)
                -- Registro("time Restante: " .. tiempoRestante7)
                if motion ~= nil then -- Conocer estado de SM
                    local securityThreat = _storage.get_string("securityThreat7")
                    setItemValue(motion, true)
                    setItemValue(securityThreat, true)
                end

                if _storage.get_number("accion7") == ACCION_SCAN then
                ...
                if _storage.get_number("accion7") == ACCION_LIBRE then
                    CancelTimer()
                    Rutina_On_Thermo()
                    ActualizarAccion(ACCION_OCUPADO)
                    Validar("timerAccion7", "Ocupado")
                ...
                if _storage.get_number("accion7") == ACCION_CICLO_APAGADO then
                    CancelTimer()
                    ActualizarAccion(ACCION_OCUPADO)
                    Validar("timerAccion7","Ocupado")
                    Validar("scanCycle7", "Ocupado")
                    if _storage.get_string("Mode7") ~= MODO_APAGADO then
                ...
                if ValidateRemainingTime(20) then
                    ...
                    if _storage.get_number("accion7") == ACCION_AUTOOFF then
                        CancelTimer()
                        ActualizarAccion(ACCION_SCAN)
                        Validar("timerAccion7", "Scan")
                        StartTimer(Tiempo1)
                        return
                    end
                end
            ...
            if sensor_id.value == false then
                ...
                if _storage.get_number("accion7") == ACCION_OCUPADO then
                    CancelTimer()
                    ActualizarAccion(ACCION_SCAN)
                    Validar("timerAccion7", "Scan")
                    Validar("scanCycle7", "scan_One")
                    StartTimer(Tiempo1)
                    return
                end
            end
...
function EventoMovDeLibre()
    ...
    Validar("timerAccion7", "CicloApagado")
    Validar("scanCycle7", "CicloApagado")
...
function CicloApagado()
    Registro("#7",rojo)
        if (_storage.get_number("accion7") == ACCION_SCAN) and (_storage.get_string("Modo7") ~= MODO_APAGADO) then
                    RoutineOffLuces(devicesOn)
                    RoutineOffLuces(devicesOff)
        end
        CancelTimer()
...
function StartTimer(timerduration)
    local counting = VariablesDeInicio("Counting7", "0")
    if counting == '1' then -- se dejan las comillas para no reemplazar este valor en las demas paginas del code Python
        return false
    end
    Validar("TimerDuration7", timerduration)
    return StartTimeralways()
end

function StartTimeralways()
    local duration = VariablesDeInicio("TimerDuration7", 30)
    local dueTimestamp1 = os.time() + duration
    Validar("dueTimestamp7", dueTimestamp1)
    local status = RemainingUpgrade()
    Validar("Counting7", '1') -- se dejan las comillas para no reemplazar este valor en las demas paginas del code Python
    if data.TimerID ~= "" and status then
        local timerID = data.TimerID
        _storage.set_string("TimerID7", tostring(timerID))
        _timer.set_timeout_with_id(10000, tostring(timerID),
            "HUB:aul.aula_inteligente/scripts/rooms/room7",
            { arg_name = "timer" })
        return true
    else
        Registro("timer sin id")
        local timerID = _timer.set_timeout(10000, "HUB:aul.aula_inteligente/scripts/rooms/room7",
            { arg_name = "timer" })
        _storage.set_string("TimerID7", tostring(timerID))
        return true
    end
...
function TiempoTranscurrido()
    local Timerload = _storage.get_string("remaining7") or 0
...
function CancelTimer()
    ...
    Remaining_ant = 0
    Validar("Counting7", "0")
    Validar("dueTimestamp7", 0)
    Validar("remaining7", "0")
    Validar("TimerDuration7", 0)
    return true
end
...
function RemainingUpgrade()
    local dueTimestamp1 = VariablesDeInicio("dueTimestamp7", 0)
    ...
    Validar("remaining7", "TR" .. restante)
    return remaining > 0
end

function ValidateRemainingTime(time)
    local timercompare = _storage.get_number("TimerDuration7") or 0
...
---- Main and Tick7 :: Funiciones principales
function Main()
    ...
    Registro("::: Manejos Internos ::: Aul7",verde)
    -- Registro("timerId: " .. _json.encode(data.TimerID),verde)
    TimerHoy()
    Registro("Tiempo Scaner= ".. Tiempo1)
    Registro("Tiempo TiempoScanH= ".. Tiempo2)
    Registro("Modo plugin: " .. _json.encode(data.modo).." |-| plugin status and TimerAccion7: " .. data.acciontext,amarillo)
    Registro("ScanCycle1: " .. _json.encode(data.scancycle),verde)
    Registro("TiempoAnterior_7: " .. _json.encode(data.previousTimer),amarillo)
    Registro("TiempoPosterior_7: " .. _json.encode(TimerRemainingV),amarillo)
    ...
    Validar("previousTimer7", tonumber(TimerRemainingV))
...
function Tick7()
    ...
    local counting = VariablesDeInicio("Counting7", "0")
    local timerAccion = VariablesDeInicio("timerAccion7", "0")
...
if ModoPluginStatus then
    ...
    CancelTimer()
    ActualizarAccion(ACCION_SCAN)
    Validar("Modo7", tostring(modeButton.value)) --Actualiza estado del aul con respecto al cambio de estado en web
    Validar("scanCycle7", "scan_One")
    Validar("timerAccion7", "Scan")
...
if AccionPluginStatus then
    ...
    if EstadoTexto and EstadoTexto.value and AccionButton and AccionButton.value then
        if AccionButton.value == "Libre" and EstadoTexto.value == "Ocupado" then
            ...
            Validar("scanCycle7", "LibreForzado")
            Libre()
        end
    else
