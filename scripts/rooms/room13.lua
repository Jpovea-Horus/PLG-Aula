...
local thermostat = _storage.get_table("TERMOSTATO_M13")
local SetPointOn = _storage.get_number("SETPOINTON_M13")
local SensorMovbatt = _storage.get_table("SENSOR_MOV_BATT_M13")
local SensorMovElec = _storage.get_table("SENSOR_MOV_ELEC_M13")
local devicesOn = _storage.get_table("ACTUADORES_ON_M13") or {}
local devicesOff = _storage.get_table("ACTUADORES_OFF_M13") or {}
local masterSwitch = _storage.get_table("MASTERSWITCH_ON_M13") or {}
local ModoDispLuces = _storage.get_string("MODODISPLUCES_M13") or false
local ModoThermoInit = _storage.get_string("MOTIONACTIVATOR_M13") or false
local ModoMasterSwitch = _storage.get_string("MODOMASTERSWITCH_M13") or false

-- No usados en plugins de Aula no lleva puerta ni SetPointOFF
-- local ModoSetpoint = _storage.get_string("MODOSETPOINT_M13") or false
-- local SetPointOff = _storage.get_number("SETPOINTOFF_M13")
-- local sensor_puerta = _storage.get_string("SENSOR_PUERTA_M13")
-- local ModoOffPuertaAbierta = _storage.get_string("OFFPUERTAABIERTA_M13") or false

local ModoPluginStatus = _storage.get_string("ModoId13")
local AccionPluginStatus = _storage.get_string("AccionTextoId13")
local EstadoPluginStatus = _storage.get_string("EstadoTextoId13")
local modeButton = (_core.get_item(ModoPluginStatus))

local data = {}
    data.modo = _storage.get_string("Modo13")
    data.type = _storage.get_string("type13")
    data.accion = _storage.get_number("accion13")
    data.TimerID = _storage.get_string("TimerID13")
    data.counting = _storage.get_string("Counting13")
    data.scancycle = _storage.get_string("scanCycle13")
    data.remaining = _storage.get_string("remaining13")
    data.statustext = _storage.get_string("statusText13")
    data.acciontext = _storage.get_string("accionTexto13")
    data.timeraccion = _storage.get_string("timerAccion13")
    data.TimerIdTick = _storage.get_string("TimerIdTick13")
    data.dueTimestamp = _storage.get_number("dueTimestamp13")
    data.timerduration = _storage.get_number("TimerDuration13")
    data.Remaining_ant = _storage.get_number("Remaining_ant13")
    data.previousTimer = _storage.get_number("previousTimer13")
    data.settingAccionId = _storage.get_string("settinAccion13")
...
local TiempoScaner = math.floor((_storage.get_number("TIEMPOSCAN_M13"))/2) -- tiempo a la mitad ejecurar los tiempo del programa
...
Registro("----------------------------------")
Registro("Manejo de Tiempos Reales/2 Aul13: ",verde)
...
function Occupied()
    ...
    Validar("timerAccion13", "Ocupado")
    Validar("scanCycle13", "Ocupado")
    Rutina_Encendido()
end
...
function RoutineOnLuces(item_id)
    local id = {}
    local mode = _storage.get_string("Modo13")
...
function ActualizarAccion(s)
    ...
    local AccionTextoId = _storage.get_string("AccionTextoId13")
    local EstadoTextoId = _storage.get_string("EstadoTextoId13")
    local curValue = _storage.get_number("accion13")
    ...
    if tonumber(s) ~= tonumber(curValue) then
        ...
        if accionText then
            Validar("accionTexto13", accionText)
    ...
    if tonumber(s) ~= -2 then
        ...
        Validar("statusText13", statusText)
...
function EventMovimiento(Sensor)
    ...
    local motion = _storage.get_string("motion13")
    if Sensor ~= nil then
        ...
        for i in ipairs(Sensor) do -- Rutina FOR : conocer estado de SM
            ...
            if sensor_id.value == true then
                local tiempoRestante13 = Tiempo1 - math.abs(data.previousTimer or 0)
                -- Registro("time Restante: " .. tiempoRestante13)
                if motion ~= nil then -- Conocer estado de SM
                    local securityThreat = _storage.get_string("securityThreat13")
                    setItemValue(motion, true)
                    setItemValue(securityThreat, true)
                end

                if _storage.get_number("accion13") == ACCION_SCAN then
                ...
                if _storage.get_number("accion13") == ACCION_LIBRE then
                    CancelTimer()
                    Rutina_On_Thermo()
                    ActualizarAccion(ACCION_OCUPADO)
                    Validar("timerAccion13", "Ocupado")
                ...
                if _storage.get_number("accion13") == ACCION_CICLO_APAGADO then
                    CancelTimer()
                    ActualizarAccion(ACCION_OCUPADO)
                    Validar("timerAccion13","Ocupado")
                    Validar("scanCycle13", "Ocupado")
                    if _storage.get_string("Mode13") ~= MODO_APAGADO then
                ...
                if ValidateRemainingTime(20) then
                    ...
                    if _storage.get_number("accion13") == ACCION_AUTOOFF then
                        CancelTimer()
                        ActualizarAccion(ACCION_SCAN)
                        Validar("timerAccion13", "Scan")
                        StartTimer(Tiempo1)
                        return
                    end
                end
            ...
            if sensor_id.value == false then
                ...
                if _storage.get_number("accion13") == ACCION_OCUPADO then
                    CancelTimer()
                    ActualizarAccion(ACCION_SCAN)
                    Validar("timerAccion13", "Scan")
                    Validar("scanCycle13", "scan_One")
                    StartTimer(Tiempo1)
                    return
                end
            end
...
function EventoMovDeLibre()
    ...
    Validar("timerAccion13", "CicloApagado")
    Validar("scanCycle13", "CicloApagado")
...
function CicloApagado()
    Registro("#13",rojo)
        if (_storage.get_number("accion13") == ACCION_SCAN) and (_storage.get_string("Modo13") ~= MODO_APAGADO) then
                    RoutineOffLuces(devicesOn)
                    RoutineOffLuces(devicesOff)
        end
        CancelTimer()
...
function StartTimer(timerduration)
    local counting = VariablesDeInicio("Counting13", "0")
    if counting == '1' then -- se dejan las comillas para no reemplazar este valor en las demas paginas del code Python
        return false
    end
    Validar("TimerDuration13", timerduration)
    return StartTimeralways()
end

function StartTimeralways()
    local duration = VariablesDeInicio("TimerDuration13", 30)
    local dueTimestamp1 = os.time() + duration
    Validar("dueTimestamp13", dueTimestamp1)
    local status = RemainingUpgrade()
    Validar("Counting13", '1') -- se dejan las comillas para no reemplazar este valor en las demas paginas del code Python
    if data.TimerID ~= "" and status then
        local timerID = data.TimerID
        _storage.set_string("TimerID13", tostring(timerID))
        _timer.set_timeout_with_id(10000, tostring(timerID),
            "HUB:aul.aula_inteligente/scripts/rooms/room13",
            { arg_name = "timer" })
        return true
    else
        Registro("timer sin id")
        local timerID = _timer.set_timeout(10000, "HUB:aul.aula_inteligente/scripts/rooms/room13",
            { arg_name = "timer" })
        _storage.set_string("TimerID13", tostring(timerID))
        return true
    end
...
function TiempoTranscurrido()
    local Timerload = _storage.get_string("remaining13") or 0
...
function CancelTimer()
    ...
    Remaining_ant = 0
    Validar("Counting13", "0")
    Validar("dueTimestamp13", 0)
    Validar("remaining13", "0")
    Validar("TimerDuration13", 0)
    return true
end
...
function RemainingUpgrade()
    local dueTimestamp1 = VariablesDeInicio("dueTimestamp13", 0)
    ...
    Validar("remaining13", "TR" .. restante)
    return remaining > 0
end

function ValidateRemainingTime(time)
    local timercompare = _storage.get_number("TimerDuration13") or 0
...
---- Main and Tick13 :: Funiciones principales
function Main()
    ...
    Registro("::: Manejos Internos ::: Aul13",verde)
    -- Registro("timerId: " .. _json.encode(data.TimerID),verde)
    TimerHoy()
    Registro("Tiempo Scaner= ".. Tiempo1)
    Registro("Tiempo TiempoScanH= ".. Tiempo2)
    Registro("Modo plugin: " .. _json.encode(data.modo).." |-| plugin status and TimerAccion13: " .. data.acciontext,amarillo)
    Registro("ScanCycle1: " .. _json.encode(data.scancycle),verde)
    Registro("TiempoAnterior_13: " .. _json.encode(data.previousTimer),amarillo)
    Registro("TiempoPosterior_13: " .. _json.encode(TimerRemainingV),amarillo)
    ...
    Validar("previousTimer13", tonumber(TimerRemainingV))
...
function Tick13()
    ...
    local counting = VariablesDeInicio("Counting13", "0")
    local timerAccion = VariablesDeInicio("timerAccion13", "0")
...
if ModoPluginStatus then
    ...
    CancelTimer()
    ActualizarAccion(ACCION_SCAN)
    Validar("Modo13", tostring(modeButton.value)) --Actualiza estado del aul con respecto al cambio de estado en web
    Validar("scanCycle13", "scan_One")
    Validar("timerAccion13", "Scan")
...
if AccionPluginStatus then
    ...
    if EstadoTexto and EstadoTexto.value and AccionButton and AccionButton.value then
        if AccionButton.value == "Libre" and EstadoTexto.value == "Ocupado" then
            ...
            Validar("scanCycle13", "LibreForzado")
            Libre()
        end
    else
