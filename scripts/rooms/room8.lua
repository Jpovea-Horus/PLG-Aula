...
local thermostat = _storage.get_table("TERMOSTATO_M8")
local SetPointOn = _storage.get_number("SETPOINTON_M8")
local SensorMovbatt = _storage.get_table("SENSOR_MOV_BATT_M8")
local SensorMovElec = _storage.get_table("SENSOR_MOV_ELEC_M8")
local devicesOn = _storage.get_table("ACTUADORES_ON_M8") or {}
local devicesOff = _storage.get_table("ACTUADORES_OFF_M8") or {}
local masterSwitch = _storage.get_table("MASTERSWITCH_ON_M8") or {}
local ModoDispLuces = _storage.get_string("MODODISPLUCES_M8") or false
local ModoThermoInit = _storage.get_string("MOTIONACTIVATOR_M8") or false
local ModoMasterSwitch = _storage.get_string("MODOMASTERSWITCH_M8") or false

-- No usados en plugins de Aula no lleva puerta ni SetPointOFF
-- local ModoSetpoint = _storage.get_string("MODOSETPOINT_M8") or false
-- local SetPointOff = _storage.get_number("SETPOINTOFF_M8")
-- local sensor_puerta = _storage.get_string("SENSOR_PUERTA_M8")
-- local ModoOffPuertaAbierta = _storage.get_string("OFFPUERTAABIERTA_M8") or false
...
local data = {}
    data.modo = _storage.get_string("Modo8")
...
local TiempoScaner = math.floor((_storage.get_number("TIEMPOSCAN_M8"))/2) -- tiempo a la mitad ejecurar los tiempo del programa
...
Registro("----------------------------------")
Registro("Manejo de Tiempos Reales/2 Aul8: ",verde)
...
function Occupied()
    ...
    Validar("timerAccion8", "Ocupado")
    Validar("scanCycle8", "Ocupado")
    Rutina_Encendido()
end
...
function RoutineOnLuces(item_id)
    local id = {}
    local mode = _storage.get_string("Modo8")
...
function ActualizarAccion(s)
    ...
    if tonumber(s) ~= tonumber(curValue) then
        ...
        if accionText then
            Validar("accionTexto8", accionText)
    ...
    if tonumber(s) ~= -2 then
        ...
        Validar("statusText8", statusText)
    ...
    if Sensor ~= nil then
        ...
        for i in ipairs(Sensor) do -- Rutina FOR : conocer estado de SM
            ...
            if sensor_id.value == true then
                local tiempoRestante8 = Tiempo1 - math.abs(data.previousTimer or 0)
                -- Registro("time Restante: " .. tiempoRestante8)
                ...
                if ValidateRemainingTime(20) then
                    ...
                    if _storage.get_number("accion8") == ACCION_AUTOOFF then
                        CancelTimer()
                        ActualizarAccion(ACCION_SCAN)
                        Validar("timerAccion8", "Scan")
                        StartTimer(Tiempo1)
                        return
                    end
                end
            ...
            if sensor_id.value == false then
                ...
                if _storage.get_number("accion8") == ACCION_OCUPADO then
                    CancelTimer()
                    ActualizarAccion(ACCION_SCAN)
                    Validar("timerAccion8", "Scan")
                    Validar("scanCycle8", "scan_One")
                    StartTimer(Tiempo1)
                    return
                end
            end
...
function EventoMovDeLibre()
    ...
    Validar("timerAccion8", "CicloApagado")
    Validar("scanCycle8", "CicloApagado")
...
function CicloApagado()
    Registro("#8",rojo)
        if (_storage.get_number("accion8") == ACCION_SCAN) and (_storage.get_string("Modo8") ~= MODO_APAGADO) then
                    RoutineOffLuces(devicesOn)
                    RoutineOffLuces(devicesOff)
        end
        CancelTimer()
...
function StartTimer(timerduration)
    ...
    Validar("TimerDuration8", timerduration)
...
function StartTimeralways()
    ...
    Validar("dueTimestamp8", dueTimestamp1)
    local status = RemainingUpgrade()
    Validar("Counting8", '1') -- se dejan las comillas para no reemplazar este valor en las demas paginas del code Python
    if data.TimerID ~= "" and status then
        ...
        _timer.set_timeout_with_id(10000, tostring(timerID),
            "HUB:aul.aula_inteligente/scripts/rooms/room8",
            { arg_name = "timer" })
        return true
    else
        Registro("timer sin id")
        local timerID = _timer.set_timeout(10000, "HUB:aul.aula_inteligente/scripts/rooms/room8",
            { arg_name = "timer" })
        _storage.set_string("TimerID8", tostring(timerID))
        return true
    end
...
function CancelTimer()
    ...
    Remaining_ant = 0
    Validar("Counting8", "0")
    Validar("dueTimestamp8", 0)
    Validar("remaining8", "0")
    Validar("TimerDuration8", 0)
    return true
end
...
function RemainingUpgrade()
    ...
    Validar("remaining8", "TR" .. restante)
    return remaining > 0
end

function ValidateRemainingTime(time)
    local timercompare = _storage.get_number("TimerDuration8") or 0
...
---- Main and Tick8 :: Funiciones principales
function Main()
    ...
    Registro("::: Manejos Internos ::: Aul8",verde)
    -- Registro("timerId: " .. _json.encode(data.TimerID),verde)
    TimerHoy()
    Registro("Tiempo Scaner= ".. Tiempo1)
    Registro("Tiempo TiempoScanH= ".. Tiempo2)
    Registro("Modo plugin: " .. _json.encode(data.modo).." |-| plugin status and TimerAccion8: " .. data.acciontext,amarillo)
    Registro("ScanCycle1: " .. _json.encode(data.scancycle),verde)
    Registro("TiempoAnterior_8: " .. _json.encode(data.previousTimer),amarillo)
    Registro("TiempoPosterior_8: " .. _json.encode(TimerRemainingV),amarillo)
    ...
    Validar("previousTimer8", tonumber(TimerRemainingV))
...
if ModoPluginStatus then
    ...
    CancelTimer()
    ActualizarAccion(ACCION_SCAN)
    Validar("Modo8", tostring(modeButton.value)) --Actualiza estado del aul con respecto al cambio de estado en web
    Validar("scanCycle8", "scan_One")
    Validar("timerAccion8", "Scan")
...
if AccionPluginStatus then
    ...
    if EstadoTexto and EstadoTexto.value and AccionButton and AccionButton.value then
        if AccionButton.value == "Libre" and EstadoTexto.value == "Ocupado" then
            ...
            Validar("scanCycle8", "LibreForzado")
            Libre()
        end
    else
