...
local thermostat = _storage.get_table("TERMOSTATO_M20")
local SetPointOn = _storage.get_number("SETPOINTON_M20")
local SensorMovbatt = _storage.get_table("SENSOR_MOV_BATT_M20")
local SensorMovElec = _storage.get_table("SENSOR_MOV_ELEC_M20")
local devicesOn = _storage.get_table("ACTUADORES_ON_M20") or {}
local devicesOff = _storage.get_table("ACTUADORES_OFF_M20") or {}
local masterSwitch = _storage.get_table("MASTERSWITCH_ON_M20") or {}
local ModoDispLuces = _storage.get_string("MODODISPLUCES_M20") or false
local ModoThermoInit = _storage.get_string("MOTIONACTIVATOR_M20") or false
local ModoMasterSwitch = _storage.get_string("MODOMASTERSWITCH_M20") or false

-- No usados en plugins de Aula no lleva puerta ni SetPointOFF
-- local ModoSetpoint = _storage.get_string("MODOSETPOINT_M20") or false
-- local SetPointOff = _storage.get_number("SETPOINTOFF_M20")
-- local sensor_puerta = _storage.get_string("SENSOR_PUERTA_M20")
-- local ModoOffPuertaAbierta = _storage.get_string("OFFPUERTAABIERTA_M20") or false

local ModoPluginStatus = _storage.get_string("ModoId20")
local AccionPluginStatus = _storage.get_string("AccionTextoId20")
local EstadoPluginStatus = _storage.get_string("EstadoTextoId20")
local modeButton = (_core.get_item(ModoPluginStatus))

local data = {}
    data.modo = _storage.get_string("Modo20")
    data.type = _storage.get_string("type20")
    data.accion = _storage.get_number("accion20")
    data.TimerID = _storage.get_string("TimerID20")
    data.counting = _storage.get_string("Counting20")
    data.scancycle = _storage.get_string("scanCycle20")
    data.remaining = _storage.get_string("remaining20")
    data.statustext = _storage.get_string("statusText20")
    data.acciontext = _storage.get_string("accionTexto20")
    data.timeraccion = _storage.get_string("timerAccion20")
    data.TimerIdTick = _storage.get_string("TimerIdTick20")
    data.dueTimestamp = _storage.get_number("dueTimestamp20")
    data.timerduration = _storage.get_number("TimerDuration20")
    data.Remaining_ant = _storage.get_number("Remaining_ant20")
    data.previousTimer = _storage.get_number("previousTimer20")
    data.settingAccionId = _storage.get_string("settinAccion20")
...
local TiempoScaner = math.floor((_storage.get_number("TIEMPOSCAN_M20"))/2) -- tiempo a la mitad ejecurar los tiempo del programa
...
Registro("----------------------------------")
Registro("Manejo de Tiempos Reales/2 Aul20: ",verde)
...
function Occupied()
    ...
    Validar("timerAccion20", "Ocupado")
    Validar("scanCycle20", "Ocupado")
    Rutina_Encendido()
end
...
function RoutineOnLuces(item_id)
    local id = {}
    local mode = _storage.get_string("Modo20")
...
function ActualizarAccion(s)
    ...
    local AccionTextoId = _storage.get_string("AccionTextoId20")
    local EstadoTextoId = _storage.get_string("EstadoTextoId20")
    local curValue = _storage.get_number("accion20")
    ...
    if tonumber(s) ~= tonumber(curValue) then
        ...
        if accionText then
            Validar("accionTexto20", accionText)
    ...
    if tonumber(s) ~= -2 then
        ...
        Validar("statusText20", statusText)
...
function EventMovimiento(Sensor)
    ...
    local motion = _storage.get_string("motion20")
    if Sensor ~= nil then
        ...
        for i in ipairs(Sensor) do -- Rutina FOR : conocer estado de SM
            ...
            if sensor_id.value == true then
                local tiempoRestante20 = Tiempo1 - math.abs(data.previousTimer or 0)
                -- Registro("time Restante: " .. tiempoRestante20)
                if motion ~= nil then -- Conocer estado de SM
                    local securityThreat = _storage.get_string("securityThreat20")
                    setItemValue(motion, true)
                    setItemValue(securityThreat, true)
                end

                if _storage.get_number("accion20") == ACCION_SCAN then
                ...
                if _storage.get_number("accion20") == ACCION_LIBRE then
                    CancelTimer()
                    Rutina_On_Thermo()
                    ActualizarAccion(ACCION_OCUPADO)
                    Validar("timerAccion20", "Ocupado")
                ...
                if _storage.get_number("accion20") == ACCION_CICLO_APAGADO then
                    CancelTimer()
                    ActualizarAccion(ACCION_OCUPADO)
                    Validar("timerAccion20","Ocupado")
                    Validar("scanCycle20", "Ocupado")
                    if _storage.get_string("Mode20") ~= MODO_APAGADO then
                ...
                if ValidateRemainingTime(20) then
                    ...
                    if _storage.get_number("accion20") == ACCION_AUTOOFF then
                        CancelTimer()
                        ActualizarAccion(ACCION_SCAN)
                        Validar("timerAccion20", "Scan")
                        StartTimer(Tiempo1)
                        return
                    end
                end
            ...
            if sensor_id.value == false then
                ...
                if _storage.get_number("accion20") == ACCION_OCUPADO then
                    CancelTimer()
                    ActualizarAccion(ACCION_SCAN)
                    Validar("timerAccion20", "Scan")
                    Validar("scanCycle20", "scan_One")
                    StartTimer(Tiempo1)
                    return
                end
            end
...
function EventoMovDeLibre()
    ...
    Validar("timerAccion20", "CicloApagado")
    Validar("scanCycle20", "CicloApagado")
...
function CicloApagado()
    Registro("#20",rojo)
        if (_storage.get_number("accion20") == ACCION_SCAN) and (_storage.get_string("Modo20") ~= MODO_APAGADO) then
                    RoutineOffLuces(devicesOn)
                    RoutineOffLuces(devicesOff)
        end
        CancelTimer()
...
function StartTimer(timerduration)
    local counting = VariablesDeInicio("Counting20", "0")
    if counting == '1' then -- se dejan las comillas para no reemplazar este valor en las demas paginas del code Python
        return false
    end
    Validar("TimerDuration20", timerduration)
    return StartTimeralways()
end

function StartTimeralways()
    local duration = VariablesDeInicio("TimerDuration20", 30)
    local dueTimestamp1 = os.time() + duration
    Validar("dueTimestamp20", dueTimestamp1)
    local status = RemainingUpgrade()
    Validar("Counting20", '1') -- se dejan las comillas para no reemplazar este valor en las demas paginas del code Python
    if data.TimerID ~= "" and status then
        local timerID = data.TimerID
        _storage.set_string("TimerID20", tostring(timerID))
        _timer.set_timeout_with_id(10000, tostring(timerID),
            "HUB:aul.aula_inteligente/scripts/rooms/room20",
            { arg_name = "timer" })
        return true
    else
        Registro("timer sin id")
        local timerID = _timer.set_timeout(10000, "HUB:aul.aula_inteligente/scripts/rooms/room20",
            { arg_name = "timer" })
        _storage.set_string("TimerID20", tostring(timerID))
        return true
    end
...
function TiempoTranscurrido()
    local Timerload = _storage.get_string("remaining20") or 0
...
function CancelTimer()
    ...
    Remaining_ant = 0
    Validar("Counting20", "0")
    Validar("dueTimestamp20", 0)
    Validar("remaining20", "0")
    Validar("TimerDuration20", 0)
    return true
end
...
function RemainingUpgrade()
    local dueTimestamp1 = VariablesDeInicio("dueTimestamp20", 0)
    ...
    Validar("remaining20", "TR" .. restante)
    return remaining > 0
end

function ValidateRemainingTime(time)
    local timercompare = _storage.get_number("TimerDuration20") or 0
...
---- Main and Tick20 :: Funiciones principales
function Main()
    ...
    Registro("::: Manejos Internos ::: Aul20",verde)
    -- Registro("timerId: " .. _json.encode(data.TimerID),verde)
    TimerHoy()
    Registro("Tiempo Scaner= ".. Tiempo1)
    Registro("Tiempo TiempoScanH= ".. Tiempo2)
    Registro("Modo plugin: " .. _json.encode(data.modo).." |-| plugin status and TimerAccion20: " .. data.acciontext,amarillo)
    Registro("ScanCycle1: " .. _json.encode(data.scancycle),verde)
    Registro("TiempoAnterior_20: " .. _json.encode(data.previousTimer),amarillo)
    Registro("TiempoPosterior_20: " .. _json.encode(TimerRemainingV),amarillo)
    ...
    Validar("previousTimer20", tonumber(TimerRemainingV))
...
function Tick20()
    ...
    local counting = VariablesDeInicio("Counting20", "0")
    local timerAccion = VariablesDeInicio("timerAccion20", "0")
...
if ModoPluginStatus then
    ...
    CancelTimer()
    ActualizarAccion(ACCION_SCAN)
    Validar("Modo20", tostring(modeButton.value)) --Actualiza estado del aul con respecto al cambio de estado en web
    Validar("scanCycle20", "scan_One")
    Validar("timerAccion20", "Scan")
...
if AccionPluginStatus then
    ...
    if EstadoTexto and EstadoTexto.value and AccionButton and AccionButton.value then
        if AccionButton.value == "Libre" and EstadoTexto.value == "Ocupado" then
            ...
            Validar("scanCycle20", "LibreForzado")
            Libre()
        end
    else
