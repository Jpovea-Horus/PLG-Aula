...
local thermostat = _storage.get_table("TERMOSTATO_M11")
local SetPointOn = _storage.get_number("SETPOINTON_M11")
local SensorMovbatt = _storage.get_table("SENSOR_MOV_BATT_M11")
local SensorMovElec = _storage.get_table("SENSOR_MOV_ELEC_M11")
local devicesOn = _storage.get_table("ACTUADORES_ON_M11") or {}
local devicesOff = _storage.get_table("ACTUADORES_OFF_M11") or {}
local masterSwitch = _storage.get_table("MASTERSWITCH_ON_M11") or {}
local ModoDispLuces = _storage.get_string("MODODISPLUCES_M11") or false
local ModoThermoInit = _storage.get_string("MOTIONACTIVATOR_M11") or false
local ModoMasterSwitch = _storage.get_string("MODOMASTERSWITCH_M11") or false

-- No usados en plugins de Aula no lleva puerta ni SetPointOFF
-- local ModoSetpoint = _storage.get_string("MODOSETPOINT_M11") or false
-- local SetPointOff = _storage.get_number("SETPOINTOFF_M11")
-- local sensor_puerta = _storage.get_string("SENSOR_PUERTA_M11")
-- local ModoOffPuertaAbierta = _storage.get_string("OFFPUERTAABIERTA_M11") or false

local ModoPluginStatus = _storage.get_string("ModoId11")
local AccionPluginStatus = _storage.get_string("AccionTextoId11")
local EstadoPluginStatus = _storage.get_string("EstadoTextoId11")
local modeButton = (_core.get_item(ModoPluginStatus))

local data = {}
    data.modo = _storage.get_string("Modo11")
    data.type = _storage.get_string("type11")
    data.accion = _storage.get_number("accion11")
    data.TimerID = _storage.get_string("TimerID11")
    data.counting = _storage.get_string("Counting11")
    data.scancycle = _storage.get_string("scanCycle11")
    data.remaining = _storage.get_string("remaining11")
    data.statustext = _storage.get_string("statusText11")
    data.acciontext = _storage.get_string("accionTexto11")
    data.timeraccion = _storage.get_string("timerAccion11")
    data.TimerIdTick = _storage.get_string("TimerIdTick11")
    data.dueTimestamp = _storage.get_number("dueTimestamp11")
    data.timerduration = _storage.get_number("TimerDuration11")
    data.Remaining_ant = _storage.get_number("Remaining_ant11")
    data.previousTimer = _storage.get_number("previousTimer11")
    data.settingAccionId = _storage.get_string("settinAccion11")
...
local TiempoScaner = math.floor((_storage.get_number("TIEMPOSCAN_M11"))/2) -- tiempo a la mitad ejecurar los tiempo del programa
...
Registro("----------------------------------")
Registro("Manejo de Tiempos Reales/2 Aul11: ",verde)
...
function Occupied()
    ...
    Validar("timerAccion11", "Ocupado")
    Validar("scanCycle11", "Ocupado")
    Rutina_Encendido()
end
...
function RoutineOnLuces(item_id)
    local id = {}
    local mode = _storage.get_string("Modo11")
...
function ActualizarAccion(s)
    ...
    local AccionTextoId = _storage.get_string("AccionTextoId11")
    local EstadoTextoId = _storage.get_string("EstadoTextoId11")
    local curValue = _storage.get_number("accion11")
    ...
    if tonumber(s) ~= tonumber(curValue) then
        ...
        if accionText then
            Validar("accionTexto11", accionText)
    ...
    if tonumber(s) ~= -2 then
        ...
        Validar("statusText11", statusText)
...
function EventMovimiento(Sensor)
    ...
    local motion = _storage.get_string("motion11")
    if Sensor ~= nil then
        ...
        for i in ipairs(Sensor) do -- Rutina FOR : conocer estado de SM
            ...
            if sensor_id.value == true then
                local tiempoRestante11 = Tiempo1 - math.abs(data.previousTimer or 0)
                -- Registro("time Restante: " .. tiempoRestante11)
                if motion ~= nil then -- Conocer estado de SM
                    local securityThreat = _storage.get_string("securityThreat11")
                    setItemValue(motion, true)
                    setItemValue(securityThreat, true)
                end

                if _storage.get_number("accion11") == ACCION_SCAN then
                ...
                if _storage.get_number("accion11") == ACCION_LIBRE then
                    CancelTimer()
                    Rutina_On_Thermo()
                    ActualizarAccion(ACCION_OCUPADO)
                    Validar("timerAccion11", "Ocupado")
                ...
                if _storage.get_number("accion11") == ACCION_CICLO_APAGADO then
                    CancelTimer()
                    ActualizarAccion(ACCION_OCUPADO)
                    Validar("timerAccion11","Ocupado")
                    Validar("scanCycle11", "Ocupado")
                    if _storage.get_string("Mode11") ~= MODO_APAGADO then
                ...
                if ValidateRemainingTime(20) then
                    ...
                    if _storage.get_number("accion11") == ACCION_AUTOOFF then
                        CancelTimer()
                        ActualizarAccion(ACCION_SCAN)
                        Validar("timerAccion11", "Scan")
                        StartTimer(Tiempo1)
                        return
                    end
                end
            ...
            if sensor_id.value == false then
                ...
                if _storage.get_number("accion11") == ACCION_OCUPADO then
                    CancelTimer()
                    ActualizarAccion(ACCION_SCAN)
                    Validar("timerAccion11", "Scan")
                    Validar("scanCycle11", "scan_One")
                    StartTimer(Tiempo1)
                    return
                end
            end
...
function EventoMovDeLibre()
    ...
    Validar("timerAccion11", "CicloApagado")
    Validar("scanCycle11", "CicloApagado")
...
function CicloApagado()
    Registro("#11",rojo)
        if (_storage.get_number("accion11") == ACCION_SCAN) and (_storage.get_string("Modo11") ~= MODO_APAGADO) then
                    RoutineOffLuces(devicesOn)
                    RoutineOffLuces(devicesOff)
        end
        CancelTimer()
...
function StartTimer(timerduration)
    local counting = VariablesDeInicio("Counting11", "0")
    if counting == '1' then -- se dejan las comillas para no reemplazar este valor en las demas paginas del code Python
        return false
    end
    Validar("TimerDuration11", timerduration)
    return StartTimeralways()
end

function StartTimeralways()
    local duration = VariablesDeInicio("TimerDuration11", 30)
    local dueTimestamp1 = os.time() + duration
    Validar("dueTimestamp11", dueTimestamp1)
    local status = RemainingUpgrade()
    Validar("Counting11", '1') -- se dejan las comillas para no reemplazar este valor en las demas paginas del code Python
    if data.TimerID ~= "" and status then
        local timerID = data.TimerID
        _storage.set_string("TimerID11", tostring(timerID))
        _timer.set_timeout_with_id(10000, tostring(timerID),
            "HUB:aul.aula_inteligente/scripts/rooms/room11",
            { arg_name = "timer" })
        return true
    else
        Registro("timer sin id")
        local timerID = _timer.set_timeout(10000, "HUB:aul.aula_inteligente/scripts/rooms/room11",
            { arg_name = "timer" })
        _storage.set_string("TimerID11", tostring(timerID))
        return true
    end
...
function TiempoTranscurrido()
    local Timerload = _storage.get_string("remaining11") or 0
...
function CancelTimer()
    ...
    Remaining_ant = 0
    Validar("Counting11", "0")
    Validar("dueTimestamp11", 0)
    Validar("remaining11", "0")
    Validar("TimerDuration11", 0)
    return true
end
...
function RemainingUpgrade()
    local dueTimestamp1 = VariablesDeInicio("dueTimestamp11", 0)
    ...
    Validar("remaining11", "TR" .. restante)
    return remaining > 0
end

function ValidateRemainingTime(time)
    local timercompare = _storage.get_number("TimerDuration11") or 0
...
---- Main and Tick11 :: Funiciones principales
function Main()
    ...
    Registro("::: Manejos Internos ::: Aul11",verde)
    -- Registro("timerId: " .. _json.encode(data.TimerID),verde)
    TimerHoy()
    Registro("Tiempo Scaner= ".. Tiempo1)
    Registro("Tiempo TiempoScanH= ".. Tiempo2)
    Registro("Modo plugin: " .. _json.encode(data.modo).." |-| plugin status and TimerAccion11: " .. data.acciontext,amarillo)
    Registro("ScanCycle1: " .. _json.encode(data.scancycle),verde)
    Registro("TiempoAnterior_11: " .. _json.encode(data.previousTimer),amarillo)
    Registro("TiempoPosterior_11: " .. _json.encode(TimerRemainingV),amarillo)
    ...
    Validar("previousTimer11", tonumber(TimerRemainingV))
...
function Tick11()
    ...
    local counting = VariablesDeInicio("Counting11", "0")
    local timerAccion = VariablesDeInicio("timerAccion11", "0")
...
if ModoPluginStatus then
    ...
    CancelTimer()
    ActualizarAccion(ACCION_SCAN)
    Validar("Modo11", tostring(modeButton.value)) --Actualiza estado del aul con respecto al cambio de estado en web
    Validar("scanCycle11", "scan_One")
    Validar("timerAccion11", "Scan")
...
if AccionPluginStatus then
    ...
    if EstadoTexto and EstadoTexto.value and AccionButton and AccionButton.value then
        if AccionButton.value == "Libre" and EstadoTexto.value == "Ocupado" then
            ...
            Validar("scanCycle11", "LibreForzado")
            Libre()
        end
    else
