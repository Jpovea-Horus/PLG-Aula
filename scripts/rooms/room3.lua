...
Registro("----------------------------------")
Registro("Manejo de Tiempos Reales/2 Aul3: ",verde)
        ...
        for i in ipairs(Sensor) do -- Rutina FOR : conocer estado de SM
            ...
            if sensor_id.value == true then
                ...
                if _storage.get_number("accion3") == ACCION_CICLO_APAGADO then
                    ...
                    Validar("timerAccion3","Ocupado")
                    Validar("scanCycle3", "Ocupado")
                    if _storage.get_string("Mode3") ~= MODO_APAGADO then
                ...
                if ValidateRemainingTime(20) then
                    ...
                    if _storage.get_number("accion3") == ACCION_AUTOOFF then
                        CancelTimer()
            ...
            if sensor_id.value == false then
                ...
                if _storage.get_number("accion3") == ACCION_OCUPADO then
                    CancelTimer()
                    ActualizarAccion(ACCION_SCAN)
                    Validar("timerAccion3", "Scan")
...
function CicloApagado()
    Registro("#3",rojo)
        if (_storage.get_number("accion3") == ACCION_SCAN) and (_storage.get_string("Modo3") ~= MODO_APAGADO) then
                    RoutineOffLuces(devicesOn)
                    RoutineOffLuces(devicesOff)
        end
        CancelTimer()
...
function StartTimeralways()
    ...
    if data.TimerID ~= "" and status then
        ...
        _timer.set_timeout_with_id(10000, tostring(timerID),
            "HUB:aul.aula_inteligente/scripts/rooms/room3",
            { arg_name = "timer" })
        return true
    else
        Registro("timer sin id")
        local timerID = _timer.set_timeout(10000, "HUB:aul.aula_inteligente/scripts/rooms/room3",
            { arg_name = "timer" })
        _storage.set_string("TimerID3", tostring(timerID))
        return true
    end
...
function ValidateRemainingTime(time)
    local timercompare = _storage.get_number("TimerDuration3") or 0
...
function Main()
    ...
    Registro("::: Manejos Internos ::: Aul3",verde)
...
if ModoPluginStatus then
    ...
    CancelTimer()
    ActualizarAccion(ACCION_SCAN)
    Validar("Modo3", tostring(modeButton.value)) --Actualiza estado del aul con respecto al cambio de estado en web
    Validar("scanCycle3", "scan_One")
    Validar("timerAccion3", "Scan")
