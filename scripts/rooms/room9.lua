...
local thermostat = _storage.get_table("TERMOSTATO_M9")
local SetPointOn = _storage.get_number("SETPOINTON_M9")
local SensorMovbatt = _storage.get_table("SENSOR_MOV_BATT_M9")
local SensorMovElec = _storage.get_table("SENSOR_MOV_ELEC_M9")
local devicesOn = _storage.get_table("ACTUADORES_ON_M9") or {}
local devicesOff = _storage.get_table("ACTUADORES_OFF_M9") or {}
local masterSwitch = _storage.get_table("MASTERSWITCH_ON_M9") or {}
local ModoDispLuces = _storage.get_string("MODODISPLUCES_M9") or false
local ModoThermoInit = _storage.get_string("MOTIONACTIVATOR_M9") or false
local ModoMasterSwitch = _storage.get_string("MODOMASTERSWITCH_M9") or false

-- No usados en plugins de Aula no lleva puerta ni SetPointOFF
-- local ModoSetpoint = _storage.get_string("MODOSETPOINT_M9") or false
-- local SetPointOff = _storage.get_number("SETPOINTOFF_M9")
-- local sensor_puerta = _storage.get_string("SENSOR_PUERTA_M9")
-- local ModoOffPuertaAbierta = _storage.get_string("OFFPUERTAABIERTA_M9") or false

local ModoPluginStatus = _storage.get_string("ModoId9")
local AccionPluginStatus = _storage.get_string("AccionTextoId9")
local EstadoPluginStatus = _storage.get_string("EstadoTextoId9")
local modeButton = (_core.get_item(ModoPluginStatus))

local data = {}
    data.modo = _storage.get_string("Modo9")
    data.type = _storage.get_string("type9")
    data.accion = _storage.get_number("accion9")
    data.TimerID = _storage.get_string("TimerID9")
    data.counting = _storage.get_string("Counting9")
    data.scancycle = _storage.get_string("scanCycle9")
    data.remaining = _storage.get_string("remaining9")
    data.statustext = _storage.get_string("statusText9")
    data.acciontext = _storage.get_string("accionTexto9")
    data.timeraccion = _storage.get_string("timerAccion9")
    data.TimerIdTick = _storage.get_string("TimerIdTick9")
    data.dueTimestamp = _storage.get_number("dueTimestamp9")
    data.timerduration = _storage.get_number("TimerDuration9")
    data.Remaining_ant = _storage.get_number("Remaining_ant9")
    data.previousTimer = _storage.get_number("previousTimer9")
    data.settingAccionId = _storage.get_string("settinAccion9")
...
local TiempoScaner = math.floor((_storage.get_number("TIEMPOSCAN_M9"))/2) -- tiempo a la mitad ejecurar los tiempo del programa
...
Registro("----------------------------------")
Registro("Manejo de Tiempos Reales/2 Aul9: ",verde)
...
function Occupied()
    ...
    Validar("timerAccion9", "Ocupado")
    Validar("scanCycle9", "Ocupado")
    Rutina_Encendido()
end
...
function RoutineOnLuces(item_id)
    local id = {}
    local mode = _storage.get_string("Modo9")
...
function ActualizarAccion(s)
    ...
    local AccionTextoId = _storage.get_string("AccionTextoId9")
    local EstadoTextoId = _storage.get_string("EstadoTextoId9")
    local curValue = _storage.get_number("accion9")
    ...
    if tonumber(s) ~= tonumber(curValue) then
        ...
        if accionText then
            Validar("accionTexto9", accionText)
    ...
    if tonumber(s) ~= -2 then
        ...
        Validar("statusText9", statusText)
...
function EventMovimiento(Sensor)
    ...
    local motion = _storage.get_string("motion9")
    if Sensor ~= nil then
        ...
        for i in ipairs(Sensor) do -- Rutina FOR : conocer estado de SM
            ...
            if sensor_id.value == true then
                local tiempoRestante9 = Tiempo1 - math.abs(data.previousTimer or 0)
                -- Registro("time Restante: " .. tiempoRestante9)
                if motion ~= nil then -- Conocer estado de SM
                    local securityThreat = _storage.get_string("securityThreat9")
                    setItemValue(motion, true)
                    setItemValue(securityThreat, true)
                end

                if _storage.get_number("accion9") == ACCION_SCAN then
                ...
                if _storage.get_number("accion9") == ACCION_LIBRE then
                    CancelTimer()
                    Rutina_On_Thermo()
                    ActualizarAccion(ACCION_OCUPADO)
                    Validar("timerAccion9", "Ocupado")
                ...
                if _storage.get_number("accion9") == ACCION_CICLO_APAGADO then
                    CancelTimer()
                    ActualizarAccion(ACCION_OCUPADO)
                    Validar("timerAccion9","Ocupado")
                    Validar("scanCycle9", "Ocupado")
                    if _storage.get_string("Mode9") ~= MODO_APAGADO then
                ...
                if ValidateRemainingTime(20) then
                    ...
                    if _storage.get_number("accion9") == ACCION_AUTOOFF then
                        CancelTimer()
                        ActualizarAccion(ACCION_SCAN)
                        Validar("timerAccion9", "Scan")
                        StartTimer(Tiempo1)
                        return
                    end
                end
            ...
            if sensor_id.value == false then
                ...
                if _storage.get_number("accion9") == ACCION_OCUPADO then
                    CancelTimer()
                    ActualizarAccion(ACCION_SCAN)
                    Validar("timerAccion9", "Scan")
                    Validar("scanCycle9", "scan_One")
                    StartTimer(Tiempo1)
                    return
                end
            end
...
function EventoMovDeLibre()
    ...
    Validar("timerAccion9", "CicloApagado")
    Validar("scanCycle9", "CicloApagado")
...
function CicloApagado()
    Registro("#9",rojo)
        if (_storage.get_number("accion9") == ACCION_SCAN) and (_storage.get_string("Modo9") ~= MODO_APAGADO) then
                    RoutineOffLuces(devicesOn)
                    RoutineOffLuces(devicesOff)
        end
        CancelTimer()
...
function StartTimer(timerduration)
    local counting = VariablesDeInicio("Counting9", "0")
    if counting == '1' then -- se dejan las comillas para no reemplazar este valor en las demas paginas del code Python
        return false
    end
    Validar("TimerDuration9", timerduration)
    return StartTimeralways()
end

function StartTimeralways()
    local duration = VariablesDeInicio("TimerDuration9", 30)
    local dueTimestamp1 = os.time() + duration
    Validar("dueTimestamp9", dueTimestamp1)
    local status = RemainingUpgrade()
    Validar("Counting9", '1') -- se dejan las comillas para no reemplazar este valor en las demas paginas del code Python
    if data.TimerID ~= "" and status then
        local timerID = data.TimerID
        _storage.set_string("TimerID9", tostring(timerID))
        _timer.set_timeout_with_id(10000, tostring(timerID),
            "HUB:aul.aula_inteligente/scripts/rooms/room9",
            { arg_name = "timer" })
        return true
    else
        Registro("timer sin id")
        local timerID = _timer.set_timeout(10000, "HUB:aul.aula_inteligente/scripts/rooms/room9",
            { arg_name = "timer" })
        _storage.set_string("TimerID9", tostring(timerID))
        return true
    end
...
function TiempoTranscurrido()
    local Timerload = _storage.get_string("remaining9") or 0
...
function CancelTimer()
    ...
    Remaining_ant = 0
    Validar("Counting9", "0")
    Validar("dueTimestamp9", 0)
    Validar("remaining9", "0")
    Validar("TimerDuration9", 0)
    return true
end
...
function RemainingUpgrade()
    local dueTimestamp1 = VariablesDeInicio("dueTimestamp9", 0)
    ...
    Validar("remaining9", "TR" .. restante)
    return remaining > 0
end

function ValidateRemainingTime(time)
    local timercompare = _storage.get_number("TimerDuration9") or 0
...
---- Main and Tick9 :: Funiciones principales
function Main()
    ...
    Registro("::: Manejos Internos ::: Aul9",verde)
    -- Registro("timerId: " .. _json.encode(data.TimerID),verde)
    TimerHoy()
    Registro("Tiempo Scaner= ".. Tiempo1)
    Registro("Tiempo TiempoScanH= ".. Tiempo2)
    Registro("Modo plugin: " .. _json.encode(data.modo).." |-| plugin status and TimerAccion9: " .. data.acciontext,amarillo)
    Registro("ScanCycle1: " .. _json.encode(data.scancycle),verde)
    Registro("TiempoAnterior_9: " .. _json.encode(data.previousTimer),amarillo)
    Registro("TiempoPosterior_9: " .. _json.encode(TimerRemainingV),amarillo)
    ...
    Validar("previousTimer9", tonumber(TimerRemainingV))
...
function Tick9()
    ...
    local counting = VariablesDeInicio("Counting9", "0")
    local timerAccion = VariablesDeInicio("timerAccion9", "0")
...
if ModoPluginStatus then
    ...
    CancelTimer()
    ActualizarAccion(ACCION_SCAN)
    Validar("Modo9", tostring(modeButton.value)) --Actualiza estado del aul con respecto al cambio de estado en web
    Validar("scanCycle9", "scan_One")
    Validar("timerAccion9", "Scan")
...
if AccionPluginStatus then
    ...
    if EstadoTexto and EstadoTexto.value and AccionButton and AccionButton.value then
        if AccionButton.value == "Libre" and EstadoTexto.value == "Ocupado" then
            ...
            Validar("scanCycle9", "LibreForzado")
            Libre()
        end
    else
