...
local thermostat = _storage.get_table("TERMOSTATO_M18")
local SetPointOn = _storage.get_number("SETPOINTON_M18")
local SensorMovbatt = _storage.get_table("SENSOR_MOV_BATT_M18")
local SensorMovElec = _storage.get_table("SENSOR_MOV_ELEC_M18")
local devicesOn = _storage.get_table("ACTUADORES_ON_M18") or {}
local devicesOff = _storage.get_table("ACTUADORES_OFF_M18") or {}
local masterSwitch = _storage.get_table("MASTERSWITCH_ON_M18") or {}
local ModoDispLuces = _storage.get_string("MODODISPLUCES_M18") or false
local ModoThermoInit = _storage.get_string("MOTIONACTIVATOR_M18") or false
local ModoMasterSwitch = _storage.get_string("MODOMASTERSWITCH_M18") or false

-- No usados en plugins de Aula no lleva puerta ni SetPointOFF
-- local ModoSetpoint = _storage.get_string("MODOSETPOINT_M18") or false
-- local SetPointOff = _storage.get_number("SETPOINTOFF_M18")
-- local sensor_puerta = _storage.get_string("SENSOR_PUERTA_M18")
-- local ModoOffPuertaAbierta = _storage.get_string("OFFPUERTAABIERTA_M18") or false

local ModoPluginStatus = _storage.get_string("ModoId18")
local AccionPluginStatus = _storage.get_string("AccionTextoId18")
local EstadoPluginStatus = _storage.get_string("EstadoTextoId18")
local modeButton = (_core.get_item(ModoPluginStatus))

local data = {}
    data.modo = _storage.get_string("Modo18")
    data.type = _storage.get_string("type18")
    data.accion = _storage.get_number("accion18")
    data.TimerID = _storage.get_string("TimerID18")
    data.counting = _storage.get_string("Counting18")
    data.scancycle = _storage.get_string("scanCycle18")
    data.remaining = _storage.get_string("remaining18")
    data.statustext = _storage.get_string("statusText18")
    data.acciontext = _storage.get_string("accionTexto18")
    data.timeraccion = _storage.get_string("timerAccion18")
    data.TimerIdTick = _storage.get_string("TimerIdTick18")
    data.dueTimestamp = _storage.get_number("dueTimestamp18")
    data.timerduration = _storage.get_number("TimerDuration18")
    data.Remaining_ant = _storage.get_number("Remaining_ant18")
    data.previousTimer = _storage.get_number("previousTimer18")
    data.settingAccionId = _storage.get_string("settinAccion18")
...
local TiempoScaner = math.floor((_storage.get_number("TIEMPOSCAN_M18"))/2) -- tiempo a la mitad ejecurar los tiempo del programa
...
Registro("----------------------------------")
Registro("Manejo de Tiempos Reales/2 Aul18: ",verde)
...
function Occupied()
    ...
    Validar("timerAccion18", "Ocupado")
    Validar("scanCycle18", "Ocupado")
    Rutina_Encendido()
end
...
function RoutineOnLuces(item_id)
    local id = {}
    local mode = _storage.get_string("Modo18")
...
function ActualizarAccion(s)
    ...
    local AccionTextoId = _storage.get_string("AccionTextoId18")
    local EstadoTextoId = _storage.get_string("EstadoTextoId18")
    local curValue = _storage.get_number("accion18")
    ...
    if tonumber(s) ~= tonumber(curValue) then
        ...
        if accionText then
            Validar("accionTexto18", accionText)
    ...
    if tonumber(s) ~= -2 then
        ...
        Validar("statusText18", statusText)
...
function EventMovimiento(Sensor)
    ...
    local motion = _storage.get_string("motion18")
    if Sensor ~= nil then
        ...
        for i in ipairs(Sensor) do -- Rutina FOR : conocer estado de SM
            ...
            if sensor_id.value == true then
                local tiempoRestante18 = Tiempo1 - math.abs(data.previousTimer or 0)
                -- Registro("time Restante: " .. tiempoRestante18)
                if motion ~= nil then -- Conocer estado de SM
                    local securityThreat = _storage.get_string("securityThreat18")
                    setItemValue(motion, true)
                    setItemValue(securityThreat, true)
                end

                if _storage.get_number("accion18") == ACCION_SCAN then
                ...
                if _storage.get_number("accion18") == ACCION_LIBRE then
                    CancelTimer()
                    Rutina_On_Thermo()
                    ActualizarAccion(ACCION_OCUPADO)
                    Validar("timerAccion18", "Ocupado")
                ...
                if _storage.get_number("accion18") == ACCION_CICLO_APAGADO then
                    CancelTimer()
                    ActualizarAccion(ACCION_OCUPADO)
                    Validar("timerAccion18","Ocupado")
                    Validar("scanCycle18", "Ocupado")
                    if _storage.get_string("Mode18") ~= MODO_APAGADO then
                ...
                if ValidateRemainingTime(20) then
                    ...
                    if _storage.get_number("accion18") == ACCION_AUTOOFF then
                        CancelTimer()
                        ActualizarAccion(ACCION_SCAN)
                        Validar("timerAccion18", "Scan")
                        StartTimer(Tiempo1)
                        return
                    end
                end
            ...
            if sensor_id.value == false then
                ...
                if _storage.get_number("accion18") == ACCION_OCUPADO then
                    CancelTimer()
                    ActualizarAccion(ACCION_SCAN)
                    Validar("timerAccion18", "Scan")
                    Validar("scanCycle18", "scan_One")
                    StartTimer(Tiempo1)
                    return
                end
            end
...
function EventoMovDeLibre()
    ...
    Validar("timerAccion18", "CicloApagado")
    Validar("scanCycle18", "CicloApagado")
...
function CicloApagado()
    Registro("#18",rojo)
        if (_storage.get_number("accion18") == ACCION_SCAN) and (_storage.get_string("Modo18") ~= MODO_APAGADO) then
                    RoutineOffLuces(devicesOn)
                    RoutineOffLuces(devicesOff)
        end
        CancelTimer()
...
function StartTimer(timerduration)
    local counting = VariablesDeInicio("Counting18", "0")
    if counting == '1' then -- se dejan las comillas para no reemplazar este valor en las demas paginas del code Python
        return false
    end
    Validar("TimerDuration18", timerduration)
    return StartTimeralways()
end

function StartTimeralways()
    local duration = VariablesDeInicio("TimerDuration18", 30)
    local dueTimestamp1 = os.time() + duration
    Validar("dueTimestamp18", dueTimestamp1)
    local status = RemainingUpgrade()
    Validar("Counting18", '1') -- se dejan las comillas para no reemplazar este valor en las demas paginas del code Python
    if data.TimerID ~= "" and status then
        local timerID = data.TimerID
        _storage.set_string("TimerID18", tostring(timerID))
        _timer.set_timeout_with_id(10000, tostring(timerID),
            "HUB:aul.aula_inteligente/scripts/rooms/room18",
            { arg_name = "timer" })
        return true
    else
        Registro("timer sin id")
        local timerID = _timer.set_timeout(10000, "HUB:aul.aula_inteligente/scripts/rooms/room18",
            { arg_name = "timer" })
        _storage.set_string("TimerID18", tostring(timerID))
        return true
    end
...
function TiempoTranscurrido()
    local Timerload = _storage.get_string("remaining18") or 0
...
function CancelTimer()
    ...
    Remaining_ant = 0
    Validar("Counting18", "0")
    Validar("dueTimestamp18", 0)
    Validar("remaining18", "0")
    Validar("TimerDuration18", 0)
    return true
end
...
function RemainingUpgrade()
    local dueTimestamp1 = VariablesDeInicio("dueTimestamp18", 0)
    ...
    Validar("remaining18", "TR" .. restante)
    return remaining > 0
end

function ValidateRemainingTime(time)
    local timercompare = _storage.get_number("TimerDuration18") or 0
...
---- Main and Tick18 :: Funiciones principales
function Main()
    ...
    Registro("::: Manejos Internos ::: Aul18",verde)
    -- Registro("timerId: " .. _json.encode(data.TimerID),verde)
    TimerHoy()
    Registro("Tiempo Scaner= ".. Tiempo1)
    Registro("Tiempo TiempoScanH= ".. Tiempo2)
    Registro("Modo plugin: " .. _json.encode(data.modo).." |-| plugin status and TimerAccion18: " .. data.acciontext,amarillo)
    Registro("ScanCycle1: " .. _json.encode(data.scancycle),verde)
    Registro("TiempoAnterior_18: " .. _json.encode(data.previousTimer),amarillo)
    Registro("TiempoPosterior_18: " .. _json.encode(TimerRemainingV),amarillo)
    ...
    Validar("previousTimer18", tonumber(TimerRemainingV))
...
function Tick18()
    ...
    local counting = VariablesDeInicio("Counting18", "0")
    local timerAccion = VariablesDeInicio("timerAccion18", "0")
...
if ModoPluginStatus then
    ...
    CancelTimer()
    ActualizarAccion(ACCION_SCAN)
    Validar("Modo18", tostring(modeButton.value)) --Actualiza estado del aul con respecto al cambio de estado en web
    Validar("scanCycle18", "scan_One")
    Validar("timerAccion18", "Scan")
...
if AccionPluginStatus then
    ...
    if EstadoTexto and EstadoTexto.value and AccionButton and AccionButton.value then
        if AccionButton.value == "Libre" and EstadoTexto.value == "Ocupado" then
            ...
            Validar("scanCycle18", "LibreForzado")
            Libre()
        end
    else
