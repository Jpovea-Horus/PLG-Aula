...
local thermostat = _storage.get_table("TERMOSTATO_M4")
local SetPointOn = _storage.get_number("SETPOINTON_M4")
local SensorMovbatt = _storage.get_table("SENSOR_MOV_BATT_M4")
local SensorMovElec = _storage.get_table("SENSOR_MOV_ELEC_M4")
local devicesOn = _storage.get_table("ACTUADORES_ON_M4") or {}
local devicesOff = _storage.get_table("ACTUADORES_OFF_M4") or {}
local masterSwitch = _storage.get_table("MASTERSWITCH_ON_M4") or {}
local ModoDispLuces = _storage.get_string("MODODISPLUCES_M4") or false
local ModoThermoInit = _storage.get_string("MOTIONACTIVATOR_M4") or false
local ModoMasterSwitch = _storage.get_string("MODOMASTERSWITCH_M4") or false

-- No usados en plugins de Aula no lleva puerta ni SetPointOFF
-- local ModoSetpoint = _storage.get_string("MODOSETPOINT_M4") or false
-- local SetPointOff = _storage.get_number("SETPOINTOFF_M4")
-- local sensor_puerta = _storage.get_string("SENSOR_PUERTA_M4")
-- local ModoOffPuertaAbierta = _storage.get_string("OFFPUERTAABIERTA_M4") or false

local ModoPluginStatus = _storage.get_string("ModoId4")
local AccionPluginStatus = _storage.get_string("AccionTextoId4")
local EstadoPluginStatus = _storage.get_string("EstadoTextoId4")
local modeButton = (_core.get_item(ModoPluginStatus))

local data = {}
    data.modo = _storage.get_string("Modo4")
    data.type = _storage.get_string("type4")
    data.accion = _storage.get_number("accion4")
    data.TimerID = _storage.get_string("TimerID4")
    data.counting = _storage.get_string("Counting4")
    data.scancycle = _storage.get_string("scanCycle4")
    data.remaining = _storage.get_string("remaining4")
    data.statustext = _storage.get_string("statusText4")
    data.acciontext = _storage.get_string("accionTexto4")
    data.timeraccion = _storage.get_string("timerAccion4")
    data.TimerIdTick = _storage.get_string("TimerIdTick4")
    data.dueTimestamp = _storage.get_number("dueTimestamp4")
    data.timerduration = _storage.get_number("TimerDuration4")
    data.Remaining_ant = _storage.get_number("Remaining_ant4")
    data.previousTimer = _storage.get_number("previousTimer4")
    data.settingAccionId = _storage.get_string("settinAccion4")
...
local TiempoScaner = math.floor((_storage.get_number("TIEMPOSCAN_M4"))/2) -- tiempo a la mitad ejecurar los tiempo del programa
...
Registro("----------------------------------")
Registro("Manejo de Tiempos Reales/2 Aul4: ",verde)
...
function Occupied()
    ...
    Validar("timerAccion4", "Ocupado")
    Validar("scanCycle4", "Ocupado")
    Rutina_Encendido()
end
...
function RoutineOnLuces(item_id)
    local id = {}
    local mode = _storage.get_string("Modo4")
...
function ActualizarAccion(s)
    ...
    local AccionTextoId = _storage.get_string("AccionTextoId4")
    local EstadoTextoId = _storage.get_string("EstadoTextoId4")
    local curValue = _storage.get_number("accion4")
    ...
    if tonumber(s) ~= tonumber(curValue) then
        ...
        if accionText then
            Validar("accionTexto4", accionText)
    ...
    if tonumber(s) ~= -2 then
        ...
        Validar("statusText4", statusText)
...
function EventMovimiento(Sensor)
    ...
    local motion = _storage.get_string("motion4")
    if Sensor ~= nil then
        ...
        for i in ipairs(Sensor) do -- Rutina FOR : conocer estado de SM
            ...
            if sensor_id.value == true then
                local tiempoRestante4 = Tiempo1 - math.abs(data.previousTimer or 0)
                -- Registro("time Restante: " .. tiempoRestante4)
                if motion ~= nil then -- Conocer estado de SM
                    local securityThreat = _storage.get_string("securityThreat4")
                    setItemValue(motion, true)
                    setItemValue(securityThreat, true)
                end

                if _storage.get_number("accion4") == ACCION_SCAN then
                ...
                if _storage.get_number("accion4") == ACCION_LIBRE then
                    CancelTimer()
                    Rutina_On_Thermo()
                    ActualizarAccion(ACCION_OCUPADO)
                    Validar("timerAccion4", "Ocupado")
                ...
                if _storage.get_number("accion4") == ACCION_CICLO_APAGADO then
                    CancelTimer()
                    ActualizarAccion(ACCION_OCUPADO)
                    Validar("timerAccion4","Ocupado")
                    Validar("scanCycle4", "Ocupado")
                    if _storage.get_string("Mode4") ~= MODO_APAGADO then
                ...
                if ValidateRemainingTime(20) then
                    ...
                    if _storage.get_number("accion4") == ACCION_AUTOOFF then
                        CancelTimer()
                        ActualizarAccion(ACCION_SCAN)
                        Validar("timerAccion4", "Scan")
                        StartTimer(Tiempo1)
                        return
                    end
                end
            ...
            if sensor_id.value == false then
                ...
                if _storage.get_number("accion4") == ACCION_OCUPADO then
                    CancelTimer()
                    ActualizarAccion(ACCION_SCAN)
                    Validar("timerAccion4", "Scan")
                    Validar("scanCycle4", "scan_One")
                    StartTimer(Tiempo1)
                    return
                end
            end
...
function EventoMovDeLibre()
    ...
    Validar("timerAccion4", "CicloApagado")
    Validar("scanCycle4", "CicloApagado")
...
function CicloApagado()
    Registro("#4",rojo)
        if (_storage.get_number("accion4") == ACCION_SCAN) and (_storage.get_string("Modo4") ~= MODO_APAGADO) then
                    RoutineOffLuces(devicesOn)
                    RoutineOffLuces(devicesOff)
        end
        CancelTimer()
...
function StartTimer(timerduration)
    local counting = VariablesDeInicio("Counting4", "0")
    if counting == '1' then -- se dejan las comillas para no reemplazar este valor en las demas paginas del code Python
        return false
    end
    Validar("TimerDuration4", timerduration)
    return StartTimeralways()
end

function StartTimeralways()
    local duration = VariablesDeInicio("TimerDuration4", 30)
    local dueTimestamp1 = os.time() + duration
    Validar("dueTimestamp4", dueTimestamp1)
    local status = RemainingUpgrade()
    Validar("Counting4", '1') -- se dejan las comillas para no reemplazar este valor en las demas paginas del code Python
    if data.TimerID ~= "" and status then
        local timerID = data.TimerID
        _storage.set_string("TimerID4", tostring(timerID))
        _timer.set_timeout_with_id(10000, tostring(timerID),
            "HUB:aul.aula_inteligente/scripts/rooms/room4",
            { arg_name = "timer" })
        return true
    else
        Registro("timer sin id")
        local timerID = _timer.set_timeout(10000, "HUB:aul.aula_inteligente/scripts/rooms/room4",
            { arg_name = "timer" })
        _storage.set_string("TimerID4", tostring(timerID))
        return true
    end
...
function TiempoTranscurrido()
    local Timerload = _storage.get_string("remaining4") or 0
...
function CancelTimer()
    ...
    Remaining_ant = 0
    Validar("Counting4", "0")
    Validar("dueTimestamp4", 0)
    Validar("remaining4", "0")
    Validar("TimerDuration4", 0)
    return true
end
...
function RemainingUpgrade()
    local dueTimestamp1 = VariablesDeInicio("dueTimestamp4", 0)
    ...
    Validar("remaining4", "TR" .. restante)
    return remaining > 0
end

function ValidateRemainingTime(time)
    local timercompare = _storage.get_number("TimerDuration4") or 0
...
---- Main and Tick4 :: Funiciones principales
function Main()
    ...
    Registro("::: Manejos Internos ::: Aul4",verde)
    -- Registro("timerId: " .. _json.encode(data.TimerID),verde)
    TimerHoy()
    Registro("Tiempo Scaner= ".. Tiempo1)
    Registro("Tiempo TiempoScanH= ".. Tiempo2)
    Registro("Modo plugin: " .. _json.encode(data.modo).." |-| plugin status and TimerAccion4: " .. data.acciontext,amarillo)
    Registro("ScanCycle1: " .. _json.encode(data.scancycle),verde)
    Registro("TiempoAnterior_4: " .. _json.encode(data.previousTimer),amarillo)
    Registro("TiempoPosterior_4: " .. _json.encode(TimerRemainingV),amarillo)
    ...
    Validar("previousTimer4", tonumber(TimerRemainingV))
...
function Tick4()
    ...
    local counting = VariablesDeInicio("Counting4", "0")
    local timerAccion = VariablesDeInicio("timerAccion4", "0")
...
if ModoPluginStatus then
    ...
    CancelTimer()
    ActualizarAccion(ACCION_SCAN)
    Validar("Modo4", tostring(modeButton.value)) --Actualiza estado del aul con respecto al cambio de estado en web
    Validar("scanCycle4", "scan_One")
    Validar("timerAccion4", "Scan")
...
if AccionPluginStatus then
    ...
    if EstadoTexto and EstadoTexto.value and AccionButton and AccionButton.value then
        if AccionButton.value == "Libre" and EstadoTexto.value == "Ocupado" then
            ...
            Validar("scanCycle4", "LibreForzado")
            Libre()
        end
    else
