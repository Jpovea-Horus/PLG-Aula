...
local thermostat = _storage.get_table("TERMOSTATO_M17")
local SetPointOn = _storage.get_number("SETPOINTON_M17")
local SensorMovbatt = _storage.get_table("SENSOR_MOV_BATT_M17")
local SensorMovElec = _storage.get_table("SENSOR_MOV_ELEC_M17")
local devicesOn = _storage.get_table("ACTUADORES_ON_M17") or {}
local devicesOff = _storage.get_table("ACTUADORES_OFF_M17") or {}
local masterSwitch = _storage.get_table("MASTERSWITCH_ON_M17") or {}
local ModoDispLuces = _storage.get_string("MODODISPLUCES_M17") or false
local ModoThermoInit = _storage.get_string("MOTIONACTIVATOR_M17") or false
local ModoMasterSwitch = _storage.get_string("MODOMASTERSWITCH_M17") or false

-- No usados en plugins de Aula no lleva puerta ni SetPointOFF
-- local ModoSetpoint = _storage.get_string("MODOSETPOINT_M17") or false
-- local SetPointOff = _storage.get_number("SETPOINTOFF_M17")
-- local sensor_puerta = _storage.get_string("SENSOR_PUERTA_M17")
-- local ModoOffPuertaAbierta = _storage.get_string("OFFPUERTAABIERTA_M17") or false

local ModoPluginStatus = _storage.get_string("ModoId17")
local AccionPluginStatus = _storage.get_string("AccionTextoId17")
local EstadoPluginStatus = _storage.get_string("EstadoTextoId17")
local modeButton = (_core.get_item(ModoPluginStatus))

local data = {}
    data.modo = _storage.get_string("Modo17")
    data.type = _storage.get_string("type17")
    data.accion = _storage.get_number("accion17")
    data.TimerID = _storage.get_string("TimerID17")
    data.counting = _storage.get_string("Counting17")
    data.scancycle = _storage.get_string("scanCycle17")
    data.remaining = _storage.get_string("remaining17")
    data.statustext = _storage.get_string("statusText17")
    data.acciontext = _storage.get_string("accionTexto17")
    data.timeraccion = _storage.get_string("timerAccion17")
    data.TimerIdTick = _storage.get_string("TimerIdTick17")
    data.dueTimestamp = _storage.get_number("dueTimestamp17")
    data.timerduration = _storage.get_number("TimerDuration17")
    data.Remaining_ant = _storage.get_number("Remaining_ant17")
    data.previousTimer = _storage.get_number("previousTimer17")
    data.settingAccionId = _storage.get_string("settinAccion17")
...
local TiempoScaner = math.floor((_storage.get_number("TIEMPOSCAN_M17"))/2) -- tiempo a la mitad ejecurar los tiempo del programa
...
Registro("----------------------------------")
Registro("Manejo de Tiempos Reales/2 Aul17: ",verde)
...
function Occupied()
    ...
    Validar("timerAccion17", "Ocupado")
    Validar("scanCycle17", "Ocupado")
    Rutina_Encendido()
end
...
function RoutineOnLuces(item_id)
    local id = {}
    local mode = _storage.get_string("Modo17")
...
function ActualizarAccion(s)
    ...
    local AccionTextoId = _storage.get_string("AccionTextoId17")
    local EstadoTextoId = _storage.get_string("EstadoTextoId17")
    local curValue = _storage.get_number("accion17")
    ...
    if tonumber(s) ~= tonumber(curValue) then
        ...
        if accionText then
            Validar("accionTexto17", accionText)
    ...
    if tonumber(s) ~= -2 then
        ...
        Validar("statusText17", statusText)
...
function EventMovimiento(Sensor)
    ...
    local motion = _storage.get_string("motion17")
    if Sensor ~= nil then
        ...
        for i in ipairs(Sensor) do -- Rutina FOR : conocer estado de SM
            ...
            if sensor_id.value == true then
                local tiempoRestante17 = Tiempo1 - math.abs(data.previousTimer or 0)
                -- Registro("time Restante: " .. tiempoRestante17)
                if motion ~= nil then -- Conocer estado de SM
                    local securityThreat = _storage.get_string("securityThreat17")
                    setItemValue(motion, true)
                    setItemValue(securityThreat, true)
                end

                if _storage.get_number("accion17") == ACCION_SCAN then
                ...
                if _storage.get_number("accion17") == ACCION_LIBRE then
                    CancelTimer()
                    Rutina_On_Thermo()
                    ActualizarAccion(ACCION_OCUPADO)
                    Validar("timerAccion17", "Ocupado")
                ...
                if _storage.get_number("accion17") == ACCION_CICLO_APAGADO then
                    CancelTimer()
                    ActualizarAccion(ACCION_OCUPADO)
                    Validar("timerAccion17","Ocupado")
                    Validar("scanCycle17", "Ocupado")
                    if _storage.get_string("Mode17") ~= MODO_APAGADO then
                ...
                if ValidateRemainingTime(20) then
                    ...
                    if _storage.get_number("accion17") == ACCION_AUTOOFF then
                        CancelTimer()
                        ActualizarAccion(ACCION_SCAN)
                        Validar("timerAccion17", "Scan")
                        StartTimer(Tiempo1)
                        return
                    end
                end
            ...
            if sensor_id.value == false then
                ...
                if _storage.get_number("accion17") == ACCION_OCUPADO then
                    CancelTimer()
                    ActualizarAccion(ACCION_SCAN)
                    Validar("timerAccion17", "Scan")
                    Validar("scanCycle17", "scan_One")
                    StartTimer(Tiempo1)
                    return
                end
            end
...
function EventoMovDeLibre()
    ...
    Validar("timerAccion17", "CicloApagado")
    Validar("scanCycle17", "CicloApagado")
...
function CicloApagado()
    Registro("#17",rojo)
        if (_storage.get_number("accion17") == ACCION_SCAN) and (_storage.get_string("Modo17") ~= MODO_APAGADO) then
                    RoutineOffLuces(devicesOn)
                    RoutineOffLuces(devicesOff)
        end
        CancelTimer()
...
function StartTimer(timerduration)
    local counting = VariablesDeInicio("Counting17", "0")
    if counting == '1' then -- se dejan las comillas para no reemplazar este valor en las demas paginas del code Python
        return false
    end
    Validar("TimerDuration17", timerduration)
    return StartTimeralways()
end

function StartTimeralways()
    local duration = VariablesDeInicio("TimerDuration17", 30)
    local dueTimestamp1 = os.time() + duration
    Validar("dueTimestamp17", dueTimestamp1)
    local status = RemainingUpgrade()
    Validar("Counting17", '1') -- se dejan las comillas para no reemplazar este valor en las demas paginas del code Python
    if data.TimerID ~= "" and status then
        local timerID = data.TimerID
        _storage.set_string("TimerID17", tostring(timerID))
        _timer.set_timeout_with_id(10000, tostring(timerID),
            "HUB:aul.aula_inteligente/scripts/rooms/room17",
            { arg_name = "timer" })
        return true
    else
        Registro("timer sin id")
        local timerID = _timer.set_timeout(10000, "HUB:aul.aula_inteligente/scripts/rooms/room17",
            { arg_name = "timer" })
        _storage.set_string("TimerID17", tostring(timerID))
        return true
    end
...
function TiempoTranscurrido()
    local Timerload = _storage.get_string("remaining17") or 0
...
function CancelTimer()
    ...
    Remaining_ant = 0
    Validar("Counting17", "0")
    Validar("dueTimestamp17", 0)
    Validar("remaining17", "0")
    Validar("TimerDuration17", 0)
    return true
end
...
function RemainingUpgrade()
    local dueTimestamp1 = VariablesDeInicio("dueTimestamp17", 0)
    ...
    Validar("remaining17", "TR" .. restante)
    return remaining > 0
end

function ValidateRemainingTime(time)
    local timercompare = _storage.get_number("TimerDuration17") or 0
...
---- Main and Tick17 :: Funiciones principales
function Main()
    ...
    Registro("::: Manejos Internos ::: Aul17",verde)
    -- Registro("timerId: " .. _json.encode(data.TimerID),verde)
    TimerHoy()
    Registro("Tiempo Scaner= ".. Tiempo1)
    Registro("Tiempo TiempoScanH= ".. Tiempo2)
    Registro("Modo plugin: " .. _json.encode(data.modo).." |-| plugin status and TimerAccion17: " .. data.acciontext,amarillo)
    Registro("ScanCycle1: " .. _json.encode(data.scancycle),verde)
    Registro("TiempoAnterior_17: " .. _json.encode(data.previousTimer),amarillo)
    Registro("TiempoPosterior_17: " .. _json.encode(TimerRemainingV),amarillo)
    ...
    Validar("previousTimer17", tonumber(TimerRemainingV))
...
function Tick17()
    ...
    local counting = VariablesDeInicio("Counting17", "0")
    local timerAccion = VariablesDeInicio("timerAccion17", "0")
...
if ModoPluginStatus then
    ...
    CancelTimer()
    ActualizarAccion(ACCION_SCAN)
    Validar("Modo17", tostring(modeButton.value)) --Actualiza estado del aul con respecto al cambio de estado en web
    Validar("scanCycle17", "scan_One")
    Validar("timerAccion17", "Scan")
...
if AccionPluginStatus then
    ...
    if EstadoTexto and EstadoTexto.value and AccionButton and AccionButton.value then
        if AccionButton.value == "Libre" and EstadoTexto.value == "Ocupado" then
            ...
            Validar("scanCycle17", "LibreForzado")
            Libre()
        end
    else
