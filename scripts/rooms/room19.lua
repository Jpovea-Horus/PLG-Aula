...
local thermostat = _storage.get_table("TERMOSTATO_M19")
local SetPointOn = _storage.get_number("SETPOINTON_M19")
local SensorMovbatt = _storage.get_table("SENSOR_MOV_BATT_M19")
local SensorMovElec = _storage.get_table("SENSOR_MOV_ELEC_M19")
local devicesOn = _storage.get_table("ACTUADORES_ON_M19") or {}
local devicesOff = _storage.get_table("ACTUADORES_OFF_M19") or {}
local masterSwitch = _storage.get_table("MASTERSWITCH_ON_M19") or {}
local ModoDispLuces = _storage.get_string("MODODISPLUCES_M19") or false
local ModoThermoInit = _storage.get_string("MOTIONACTIVATOR_M19") or false
local ModoMasterSwitch = _storage.get_string("MODOMASTERSWITCH_M19") or false

-- No usados en plugins de Aula no lleva puerta ni SetPointOFF
-- local ModoSetpoint = _storage.get_string("MODOSETPOINT_M19") or false
-- local SetPointOff = _storage.get_number("SETPOINTOFF_M19")
-- local sensor_puerta = _storage.get_string("SENSOR_PUERTA_M19")
-- local ModoOffPuertaAbierta = _storage.get_string("OFFPUERTAABIERTA_M19") or false

local ModoPluginStatus = _storage.get_string("ModoId19")
local AccionPluginStatus = _storage.get_string("AccionTextoId19")
local EstadoPluginStatus = _storage.get_string("EstadoTextoId19")
local modeButton = (_core.get_item(ModoPluginStatus))

local data = {}
    data.modo = _storage.get_string("Modo19")
    data.type = _storage.get_string("type19")
    data.accion = _storage.get_number("accion19")
    data.TimerID = _storage.get_string("TimerID19")
    data.counting = _storage.get_string("Counting19")
    data.scancycle = _storage.get_string("scanCycle19")
    data.remaining = _storage.get_string("remaining19")
    data.statustext = _storage.get_string("statusText19")
    data.acciontext = _storage.get_string("accionTexto19")
    data.timeraccion = _storage.get_string("timerAccion19")
    data.TimerIdTick = _storage.get_string("TimerIdTick19")
    data.dueTimestamp = _storage.get_number("dueTimestamp19")
    data.timerduration = _storage.get_number("TimerDuration19")
    data.Remaining_ant = _storage.get_number("Remaining_ant19")
    data.previousTimer = _storage.get_number("previousTimer19")
    data.settingAccionId = _storage.get_string("settinAccion19")
...
local TiempoScaner = math.floor((_storage.get_number("TIEMPOSCAN_M19"))/2) -- tiempo a la mitad ejecurar los tiempo del programa
...
Registro("----------------------------------")
Registro("Manejo de Tiempos Reales/2 Aul19: ",verde)
...
function Occupied()
    ...
    Validar("timerAccion19", "Ocupado")
    Validar("scanCycle19", "Ocupado")
    Rutina_Encendido()
end
...
function RoutineOnLuces(item_id)
    local id = {}
    local mode = _storage.get_string("Modo19")
...
function ActualizarAccion(s)
    ...
    local AccionTextoId = _storage.get_string("AccionTextoId19")
    local EstadoTextoId = _storage.get_string("EstadoTextoId19")
    local curValue = _storage.get_number("accion19")
    ...
    if tonumber(s) ~= tonumber(curValue) then
        ...
        if accionText then
            Validar("accionTexto19", accionText)
    ...
    if tonumber(s) ~= -2 then
        ...
        Validar("statusText19", statusText)
...
function EventMovimiento(Sensor)
    ...
    local motion = _storage.get_string("motion19")
    if Sensor ~= nil then
        ...
        for i in ipairs(Sensor) do -- Rutina FOR : conocer estado de SM
            ...
            if sensor_id.value == true then
                local tiempoRestante19 = Tiempo1 - math.abs(data.previousTimer or 0)
                -- Registro("time Restante: " .. tiempoRestante19)
                if motion ~= nil then -- Conocer estado de SM
                    local securityThreat = _storage.get_string("securityThreat19")
                    setItemValue(motion, true)
                    setItemValue(securityThreat, true)
                end

                if _storage.get_number("accion19") == ACCION_SCAN then
                ...
                if _storage.get_number("accion19") == ACCION_LIBRE then
                    CancelTimer()
                    Rutina_On_Thermo()
                    ActualizarAccion(ACCION_OCUPADO)
                    Validar("timerAccion19", "Ocupado")
                ...
                if _storage.get_number("accion19") == ACCION_CICLO_APAGADO then
                    CancelTimer()
                    ActualizarAccion(ACCION_OCUPADO)
                    Validar("timerAccion19","Ocupado")
                    Validar("scanCycle19", "Ocupado")
                    if _storage.get_string("Mode19") ~= MODO_APAGADO then
                ...
                if ValidateRemainingTime(20) then
                    ...
                    if _storage.get_number("accion19") == ACCION_AUTOOFF then
                        CancelTimer()
                        ActualizarAccion(ACCION_SCAN)
                        Validar("timerAccion19", "Scan")
                        StartTimer(Tiempo1)
                        return
                    end
                end
            ...
            if sensor_id.value == false then
                ...
                if _storage.get_number("accion19") == ACCION_OCUPADO then
                    CancelTimer()
                    ActualizarAccion(ACCION_SCAN)
                    Validar("timerAccion19", "Scan")
                    Validar("scanCycle19", "scan_One")
                    StartTimer(Tiempo1)
                    return
                end
            end
...
function EventoMovDeLibre()
    ...
    Validar("timerAccion19", "CicloApagado")
    Validar("scanCycle19", "CicloApagado")
...
function CicloApagado()
    Registro("#19",rojo)
        if (_storage.get_number("accion19") == ACCION_SCAN) and (_storage.get_string("Modo19") ~= MODO_APAGADO) then
                    RoutineOffLuces(devicesOn)
                    RoutineOffLuces(devicesOff)
        end
        CancelTimer()
...
function StartTimer(timerduration)
    local counting = VariablesDeInicio("Counting19", "0")
    if counting == '1' then -- se dejan las comillas para no reemplazar este valor en las demas paginas del code Python
        return false
    end
    Validar("TimerDuration19", timerduration)
    return StartTimeralways()
end

function StartTimeralways()
    local duration = VariablesDeInicio("TimerDuration19", 30)
    local dueTimestamp1 = os.time() + duration
    Validar("dueTimestamp19", dueTimestamp1)
    local status = RemainingUpgrade()
    Validar("Counting19", '1') -- se dejan las comillas para no reemplazar este valor en las demas paginas del code Python
    if data.TimerID ~= "" and status then
        local timerID = data.TimerID
        _storage.set_string("TimerID19", tostring(timerID))
        _timer.set_timeout_with_id(10000, tostring(timerID),
            "HUB:aul.aula_inteligente/scripts/rooms/room19",
            { arg_name = "timer" })
        return true
    else
        Registro("timer sin id")
        local timerID = _timer.set_timeout(10000, "HUB:aul.aula_inteligente/scripts/rooms/room19",
            { arg_name = "timer" })
        _storage.set_string("TimerID19", tostring(timerID))
        return true
    end
...
function TiempoTranscurrido()
    local Timerload = _storage.get_string("remaining19") or 0
...
function CancelTimer()
    ...
    Remaining_ant = 0
    Validar("Counting19", "0")
    Validar("dueTimestamp19", 0)
    Validar("remaining19", "0")
    Validar("TimerDuration19", 0)
    return true
end
...
function RemainingUpgrade()
    local dueTimestamp1 = VariablesDeInicio("dueTimestamp19", 0)
    ...
    Validar("remaining19", "TR" .. restante)
    return remaining > 0
end

function ValidateRemainingTime(time)
    local timercompare = _storage.get_number("TimerDuration19") or 0
...
---- Main and Tick19 :: Funiciones principales
function Main()
    ...
    Registro("::: Manejos Internos ::: Aul19",verde)
    -- Registro("timerId: " .. _json.encode(data.TimerID),verde)
    TimerHoy()
    Registro("Tiempo Scaner= ".. Tiempo1)
    Registro("Tiempo TiempoScanH= ".. Tiempo2)
    Registro("Modo plugin: " .. _json.encode(data.modo).." |-| plugin status and TimerAccion19: " .. data.acciontext,amarillo)
    Registro("ScanCycle1: " .. _json.encode(data.scancycle),verde)
    Registro("TiempoAnterior_19: " .. _json.encode(data.previousTimer),amarillo)
    Registro("TiempoPosterior_19: " .. _json.encode(TimerRemainingV),amarillo)
    ...
    Validar("previousTimer19", tonumber(TimerRemainingV))
...
function Tick19()
    ...
    local counting = VariablesDeInicio("Counting19", "0")
    local timerAccion = VariablesDeInicio("timerAccion19", "0")
...
if ModoPluginStatus then
    ...
    CancelTimer()
    ActualizarAccion(ACCION_SCAN)
    Validar("Modo19", tostring(modeButton.value)) --Actualiza estado del aul con respecto al cambio de estado en web
    Validar("scanCycle19", "scan_One")
    Validar("timerAccion19", "Scan")
...
if AccionPluginStatus then
    ...
    if EstadoTexto and EstadoTexto.value and AccionButton and AccionButton.value then
        if AccionButton.value == "Libre" and EstadoTexto.value == "Ocupado" then
            ...
            Validar("scanCycle19", "LibreForzado")
            Libre()
        end
    else
