...
local thermostat = _storage.get_table("TERMOSTATO_M16")
local SetPointOn = _storage.get_number("SETPOINTON_M16")
local SensorMovbatt = _storage.get_table("SENSOR_MOV_BATT_M16")
local SensorMovElec = _storage.get_table("SENSOR_MOV_ELEC_M16")
local devicesOn = _storage.get_table("ACTUADORES_ON_M16") or {}
local devicesOff = _storage.get_table("ACTUADORES_OFF_M16") or {}
local masterSwitch = _storage.get_table("MASTERSWITCH_ON_M16") or {}
local ModoDispLuces = _storage.get_string("MODODISPLUCES_M16") or false
local ModoThermoInit = _storage.get_string("MOTIONACTIVATOR_M16") or false
local ModoMasterSwitch = _storage.get_string("MODOMASTERSWITCH_M16") or false

-- No usados en plugins de Aula no lleva puerta ni SetPointOFF
-- local ModoSetpoint = _storage.get_string("MODOSETPOINT_M16") or false
-- local SetPointOff = _storage.get_number("SETPOINTOFF_M16")
-- local sensor_puerta = _storage.get_string("SENSOR_PUERTA_M16")
-- local ModoOffPuertaAbierta = _storage.get_string("OFFPUERTAABIERTA_M16") or false

local ModoPluginStatus = _storage.get_string("ModoId16")
local AccionPluginStatus = _storage.get_string("AccionTextoId16")
local EstadoPluginStatus = _storage.get_string("EstadoTextoId16")
local modeButton = (_core.get_item(ModoPluginStatus))

local data = {}
    data.modo = _storage.get_string("Modo16")
    data.type = _storage.get_string("type16")
    data.accion = _storage.get_number("accion16")
    data.TimerID = _storage.get_string("TimerID16")
    data.counting = _storage.get_string("Counting16")
    data.scancycle = _storage.get_string("scanCycle16")
    data.remaining = _storage.get_string("remaining16")
    data.statustext = _storage.get_string("statusText16")
    data.acciontext = _storage.get_string("accionTexto16")
    data.timeraccion = _storage.get_string("timerAccion16")
    data.TimerIdTick = _storage.get_string("TimerIdTick16")
    data.dueTimestamp = _storage.get_number("dueTimestamp16")
    data.timerduration = _storage.get_number("TimerDuration16")
    data.Remaining_ant = _storage.get_number("Remaining_ant16")
    data.previousTimer = _storage.get_number("previousTimer16")
    data.settingAccionId = _storage.get_string("settinAccion16")
...
local TiempoScaner = math.floor((_storage.get_number("TIEMPOSCAN_M16"))/2) -- tiempo a la mitad ejecurar los tiempo del programa
...
Registro("----------------------------------")
Registro("Manejo de Tiempos Reales/2 Aul16: ",verde)
...
function Occupied()
    ...
    Validar("timerAccion16", "Ocupado")
    Validar("scanCycle16", "Ocupado")
    Rutina_Encendido()
end
...
function RoutineOnLuces(item_id)
    local id = {}
    local mode = _storage.get_string("Modo16")
...
function ActualizarAccion(s)
    ...
    local AccionTextoId = _storage.get_string("AccionTextoId16")
    local EstadoTextoId = _storage.get_string("EstadoTextoId16")
    local curValue = _storage.get_number("accion16")
    ...
    if tonumber(s) ~= tonumber(curValue) then
        ...
        if accionText then
            Validar("accionTexto16", accionText)
    ...
    if tonumber(s) ~= -2 then
        ...
        Validar("statusText16", statusText)
...
function EventMovimiento(Sensor)
    ...
    local motion = _storage.get_string("motion16")
    if Sensor ~= nil then
        ...
        for i in ipairs(Sensor) do -- Rutina FOR : conocer estado de SM
            ...
            if sensor_id.value == true then
                local tiempoRestante16 = Tiempo1 - math.abs(data.previousTimer or 0)
                -- Registro("time Restante: " .. tiempoRestante16)
                if motion ~= nil then -- Conocer estado de SM
                    local securityThreat = _storage.get_string("securityThreat16")
                    setItemValue(motion, true)
                    setItemValue(securityThreat, true)
                end

                if _storage.get_number("accion16") == ACCION_SCAN then
                ...
                if _storage.get_number("accion16") == ACCION_LIBRE then
                    CancelTimer()
                    Rutina_On_Thermo()
                    ActualizarAccion(ACCION_OCUPADO)
                    Validar("timerAccion16", "Ocupado")
                ...
                if _storage.get_number("accion16") == ACCION_CICLO_APAGADO then
                    CancelTimer()
                    ActualizarAccion(ACCION_OCUPADO)
                    Validar("timerAccion16","Ocupado")
                    Validar("scanCycle16", "Ocupado")
                    if _storage.get_string("Mode16") ~= MODO_APAGADO then
                ...
                if ValidateRemainingTime(20) then
                    ...
                    if _storage.get_number("accion16") == ACCION_AUTOOFF then
                        CancelTimer()
                        ActualizarAccion(ACCION_SCAN)
                        Validar("timerAccion16", "Scan")
                        StartTimer(Tiempo1)
                        return
                    end
                end
            ...
            if sensor_id.value == false then
                ...
                if _storage.get_number("accion16") == ACCION_OCUPADO then
                    CancelTimer()
                    ActualizarAccion(ACCION_SCAN)
                    Validar("timerAccion16", "Scan")
                    Validar("scanCycle16", "scan_One")
                    StartTimer(Tiempo1)
                    return
                end
            end
...
function EventoMovDeLibre()
    ...
    Validar("timerAccion16", "CicloApagado")
    Validar("scanCycle16", "CicloApagado")
...
function CicloApagado()
    Registro("#16",rojo)
        if (_storage.get_number("accion16") == ACCION_SCAN) and (_storage.get_string("Modo16") ~= MODO_APAGADO) then
                    RoutineOffLuces(devicesOn)
                    RoutineOffLuces(devicesOff)
        end
        CancelTimer()
...
function StartTimer(timerduration)
    local counting = VariablesDeInicio("Counting16", "0")
    if counting == '1' then -- se dejan las comillas para no reemplazar este valor en las demas paginas del code Python
        return false
    end
    Validar("TimerDuration16", timerduration)
    return StartTimeralways()
end

function StartTimeralways()
    local duration = VariablesDeInicio("TimerDuration16", 30)
    local dueTimestamp1 = os.time() + duration
    Validar("dueTimestamp16", dueTimestamp1)
    local status = RemainingUpgrade()
    Validar("Counting16", '1') -- se dejan las comillas para no reemplazar este valor en las demas paginas del code Python
    if data.TimerID ~= "" and status then
        local timerID = data.TimerID
        _storage.set_string("TimerID16", tostring(timerID))
        _timer.set_timeout_with_id(10000, tostring(timerID),
            "HUB:aul.aula_inteligente/scripts/rooms/room16",
            { arg_name = "timer" })
        return true
    else
        Registro("timer sin id")
        local timerID = _timer.set_timeout(10000, "HUB:aul.aula_inteligente/scripts/rooms/room16",
            { arg_name = "timer" })
        _storage.set_string("TimerID16", tostring(timerID))
        return true
    end
...
function TiempoTranscurrido()
    local Timerload = _storage.get_string("remaining16") or 0
...
function CancelTimer()
    ...
    Remaining_ant = 0
    Validar("Counting16", "0")
    Validar("dueTimestamp16", 0)
    Validar("remaining16", "0")
    Validar("TimerDuration16", 0)
    return true
end
...
function RemainingUpgrade()
    local dueTimestamp1 = VariablesDeInicio("dueTimestamp16", 0)
    ...
    Validar("remaining16", "TR" .. restante)
    return remaining > 0
end

function ValidateRemainingTime(time)
    local timercompare = _storage.get_number("TimerDuration16") or 0
...
---- Main and Tick16 :: Funiciones principales
function Main()
    ...
    Registro("::: Manejos Internos ::: Aul16",verde)
    -- Registro("timerId: " .. _json.encode(data.TimerID),verde)
    TimerHoy()
    Registro("Tiempo Scaner= ".. Tiempo1)
    Registro("Tiempo TiempoScanH= ".. Tiempo2)
    Registro("Modo plugin: " .. _json.encode(data.modo).." |-| plugin status and TimerAccion16: " .. data.acciontext,amarillo)
    Registro("ScanCycle1: " .. _json.encode(data.scancycle),verde)
    Registro("TiempoAnterior_16: " .. _json.encode(data.previousTimer),amarillo)
    Registro("TiempoPosterior_16: " .. _json.encode(TimerRemainingV),amarillo)
    ...
    Validar("previousTimer16", tonumber(TimerRemainingV))
...
function Tick16()
    ...
    local counting = VariablesDeInicio("Counting16", "0")
    local timerAccion = VariablesDeInicio("timerAccion16", "0")
...
if ModoPluginStatus then
    ...
    CancelTimer()
    ActualizarAccion(ACCION_SCAN)
    Validar("Modo16", tostring(modeButton.value)) --Actualiza estado del aul con respecto al cambio de estado en web
    Validar("scanCycle16", "scan_One")
    Validar("timerAccion16", "Scan")
...
if AccionPluginStatus then
    ...
    if EstadoTexto and EstadoTexto.value and AccionButton and AccionButton.value then
        if AccionButton.value == "Libre" and EstadoTexto.value == "Ocupado" then
            ...
            Validar("scanCycle16", "LibreForzado")
            Libre()
        end
    else
